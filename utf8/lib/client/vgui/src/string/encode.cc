...
int32_t g_nwMem_mbcs_to_utf8 = 0;
...
int32_t g_nwMem_utf8_to_mbcs = 0;

//缺省的代码页(936)
//1258:Vetnam
int32_t g_defaulat_code_page = 936;

int32_t g_safecheck = 1024 * 512; //512k
const char* g_errorstring_mbcs = "<ERR_MEMORY>";
const wchar_t* g_errorstring_ucs = L"<ERR_MEMORY>";

void vscii_to_ucs16(const char* mbcs_string, 
                    int32_t mbcs_string_size, 
                    wchar_t* wchar_buf, 
                    int32_t wchar_buf_size);
void ucs16_to_vscii(const wchar_t* wchar_string, 
                    int32_t wchar_char_counts, 
                    char* mbcs_string, 
                    int32_t mbcs_string_size);

//临时内存自动释放
struct stringencode_memory_t {
  ~stringencode_memory_t() {
...
void set_code_page(int32_t code_page) {
  g_defaulat_code_page=code_page;
}

int32_t get_code_page() {
  return g_defaulat_code_page;
}
...
int32_t encode_ucs_to_utf8(const wchar_t* src, 
  ...
  if (0 == src_len) {
    src_len = static_cast<int>(wcslen(src));
  }
  ...
  for (int32_t idx = 0; idx < src_len; ++idx) {
    wchar_t  cp = src[idx];
    // check there is enough destination buffer 
    // to receive this encoded unit (exit loop & return if not)
    if (destCapacity < encoded_size(cp)) {
      break;
    }

    if (cp < 0x80) {
      *dest++ = static_cast<unsigned char>(cp);
      --destCapacity;
    }
    else if (cp < 0x0800) {
      *dest++ = static_cast<unsigned char>(((cp >> 6) | 0xC0));
      *dest++ = static_cast<unsigned char>(((cp & 0x3F) | 0x80));
      destCapacity -= 2;
    }
    else if (cp < 0x10000) {
      *dest++ = static_cast<unsigned char>(((cp >> 12) | 0xE0));
      *dest++ = static_cast<unsigned char>((((cp >> 6) & 0x3F) | 0x80));
      *dest++ = static_cast<unsigned char>(((cp & 0x3F) | 0x80));
      destCapacity -= 3;
    }
    else {
      *dest++ = static_cast<unsigned char>(((cp >> 18) | 0xF0));
      *dest++ = static_cast<unsigned char>((((cp >> 12) & 0x3F) | 0x80));
      *dest++ = static_cast<unsigned char>((((cp >> 6) & 0x3F) | 0x80));
      *dest++ = static_cast<unsigned char>(((cp & 0x3F) | 0x80));
      destCapacity -= 4;
    }
...
int32_t encode_utf8_to_ucs(const unsigned char* src, 
  ...
  for (int32_t idx = 0; ((idx < src_len) && (destCapacity > 0));) {
    wchar_t  cp;
    unsigned char cu = src[idx++];

    if (cu < 0x80) {
      cp = static_cast<wchar_t>(cu);
    }
    else if (cu < 0xE0) {
      cp = ((cu & 0x1F) << 6);
      cp |= (src[idx++] & 0x3F);
...
void mbcs_to_utf8(const std::string& in, std::string& out) {
  //MBCS -> Utf16
  if (static_cast<int32_t>(in.size()) + 1 > g_nwMem_mbcs_to_utf8) {
    //safe check
    if (static_cast<int32_t>(in.size()) + 1 >= g_safecheck) {
      out = g_errorstring_mbcs;
      return;
    }

    g_nwMem_mbcs_to_utf8 = static_cast<int32_t>(in.size()) + 1;
  ...
  mbcs_to_ucs16(g_defaulat_code_page, 
                in.c_str(), 
                static_cast<int32_t>(in.size()), 
                g_pwMem_mbcs_to_utf8, 
                static_cast<int32_t>(in.size()) + 1);

  //Utf16 -> Utf8
  int32_t utf8size = static_cast<int32_t>(in.size()) * 3 + 1;
  if (utf8size > g_nMem_mbcs_to_utf8) {
    //safe check
    if(utf8size >= g_safecheck) {
      out = g_errorstring_mbcs;
      return;
    }

    g_nMem_mbcs_to_utf8 = utf8size;
  ...
  out = g_pMem_mbcs_to_utf8;
}

void utf8_to_mbcs(const std::string& in, std::string& out) {
  //Utf8->Utf16
  int32_t utf16size = static_cast<int32_t>(in.size()) + 1;
  if (utf16size > g_nwMem_utf8_to_mbcs) {
    //safe check
    if(utf16size >= g_safecheck) {
      out = g_errorstring_mbcs;
      return;
    }

    //realloc memory
    g_nwMem_utf8_to_mbcs = utf16size;
  ...
  encode_utf8_to_ucs((const unsigned char*)in.c_str(), 
                      g_pwMem_utf8_to_mbcs, 
                      g_nwMem_utf8_to_mbcs, 
                      0);

  //Utf16->mbcs
  int32_t mbcs_size = g_nwMem_utf8_to_mbcs;
  if (mbcs_size > g_nMem_utf8_to_mbcs) {
    //safe check
    if (mbcs_size >= g_safecheck) {
      out = g_errorstring_mbcs;
      return;
    }

    //realloc memory
    g_nMem_utf8_to_mbcs = mbcs_size;
  ...
  ucs16_to_mbcs(g_defaulat_code_page, 
                g_pwMem_utf8_to_mbcs, 
                static_cast<int>(wcslen(g_pwMem_utf8_to_mbcs)), 
                g_pMem_utf8_to_mbcs, 
                g_nMem_utf8_to_mbcs);

  out = g_pMem_utf8_to_mbcs;
}

//mbcs->ucs16
void mbcs_to_ucs16(int32_t code_page, 
                   const char* mbcs_string, 
                   int32_t mbcs_char_counts, 
                   wchar_t* wchar_buf, 
                   int32_t wchar_buf_size) {
  switch (code_page) {
...
void vscii_to_ucs16(const char* mbcs_string, 
  ...
  for (i = 0; i < mbcs_char_counts; ++i) {
    ...
    wchar_buf[i] = 
      g_vscii_code_page[static_cast<unsigned char>(mbcs_string[i])];
  }
  wchar_buf[i]=0;
}
...
void ucs16_to_vscii(const wchar_t* wchar_string, 
  ...
  if (!s_vscii_map_build) {
    for (i = 0; i < 0x100; ++i) {
      g_ucs16_2_vscii_map.insert(
          std::make_pair(g_vscii_code_page[i], static_cast<unsigned char>(i)));
    }
    s_vscii_map_build = true;
  }

  for (register int32_t i = 0; i < wchar_char_counts; ++i) {
    if (i >= mbcs_string_size) break;
    Ucs16_to_vscii_map::iterator it = g_ucs16_2_vscii_map.find(wchar_string[i]);
    char ch = 
      (it == g_ucs16_2_vscii_map.end()) ? 
      defaultChar : 
