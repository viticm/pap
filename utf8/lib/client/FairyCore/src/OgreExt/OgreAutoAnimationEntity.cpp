...
namespace Ogre {
    ...
    //-----------------------------------------------------------------------
	AutoAnimationEntity::AutoAnimationEntity(const Ogre::String& name, MeshPtr& mesh)
		: Entity(name, mesh)
		, mAutoUpdateAnimationStateList()
		, mLastUpdateTime()
	{
    ...
    //-----------------------------------------------------------------------
	const Ogre::String& AutoAnimationEntity::getMovableType(void) const
	{
    ...
    {
		Ogre::Real currentTime = Ogre::ControllerManager::getSingleton().getElapsedTime();
		...
		if (mLastUpdateTime != currentTime)
		{
			...
			for (it = mAutoUpdateAnimationStateList.begin(); it != itend; ++it)
			{
				...
				if (as->getEnabled())
				{
					Ogre::Real timeOffset = it->second;
    ...
    ShadowCaster::ShadowRenderableListIterator AutoAnimationEntity::getShadowVolumeRenderableIterator(
            ...
            bool extrudeVertices, Ogre::Real extrusionDistance, unsigned long flags)
    {
        _updateAutoAnimationStates();
    ...
    //-----------------------------------------------------------------------
	void AutoAnimationEntity::addAutoUpdateAnimationState(const Ogre::String& name, Ogre::Real timeOffset)
	{
    ...
    //-----------------------------------------------------------------------
	void AutoAnimationEntity::removeAutoUpdateAnimationState(const Ogre::String& name)
	{
        AnimationState* as = getAnimationState(name);
	...
	//-----------------------------------------------------------------------
	const Ogre::String& AutoAnimationEntityFactory::getType(void) const
	{
		return FACTORY_TYPE_NAME;
	}
	//-----------------------------------------------------------------------
	MovableObject* AutoAnimationEntityFactory::createInstanceImpl(const Ogre::String& name, 
