...
namespace Ogre {
    ...
    //-----------------------------------------------------------------------
    PakFileSystemArchive::PakFileSystemArchive(const Ogre::String& name, const Ogre::String& archType )
    ...
    //-----------------------------------------------------------------------
    void PakFileSystemArchive::findFiles(const Ogre::String& pattern, bool recursive, 
       Ogre::StringVector* simpleList, FileInfoList* detailList, 
        const Ogre::String& currentDir)
    {
		// parsing requires locking due to saved returns
		OGRE_LOCK_AUTO_MUTEX

		std::map<String, FILE_DESC >::iterator it;
		String pat = pattern;
		if( pat.empty() ) pat = "*";
		for( it = mPackedFileMap.begin(); it != mPackedFileMap.end(); ++ it )
		{
			if(Ogre::StringUtil::match(Ogre::String(it->second.szFileName), pat ) )
			{
				if( simpleList )
				{
    ...
    void PakFileSystemArchive::load()
    {
		...
		while( (Ogre::ulong)( lpHead - pHead ) < mdwHeadLength )
		{
			Length = *lpHead ++;
			...
			mPackedFileMap.insert( mPackedFilePair(Ogre::String(fd.szFileName), fd ) );
		}
		delete [] pHead;
		fclose( fp );
		...
		{
			//String strPackName;
			//strPackName = mBasePath + ".p" +Ogre::StringConverter::toString( n, 2, '0' );
			...
			//fseek( fp, 0, SEEK_END );
			//pd.dwFileSize = (Ogre::ulong)ftell( fp );
    ...
    {
		std::vector<PACKAGE_DESC >::iterator it;
    ...
    //-----------------------------------------------------------------------
    DataStreamPtr PakFileSystemArchive::open(const Ogre::String& filename) const
    {
		...
		std::map<String, FILE_DESC >::const_iterator fd;
		fd = mPackedFileMap.find( search );

		MemoryDataStream* stream = NULL;
		if( fd != mPackedFileMap.end() )
		{
			Ogre::ulong dwFarOffset = 0;
			std::vector<PACKAGE_DESC >::const_iterator it;
			for( it = mPackedFiles.begin(); it != mPackedFiles.end(); ++ it ) {
				if( fd->second.dwOffset < dwFarOffset + (*it).dwAvailSize ) {
					...
					//Ogre::ulong result = (Ogre::ulong)fread( pData, 1, fd->second.dwSize, (*it).lpFile );
    ...
    //-----------------------------------------------------------------------
   Ogre::StringVectorPtr PakFileSystemArchive::list(bool recursive,bool dirs)
    {
		// directory change requires locking due to saved returns
		OGRE_LOCK_AUTO_MUTEX
    ...
    //-----------------------------------------------------------------------
    FileInfoListPtr PakFileSystemArchive::listFileInfo(bool recursive,bool dirs )
    {
		// directory change requires locking due to saved returns
		OGRE_LOCK_AUTO_MUTEX
    ...
    //-----------------------------------------------------------------------
   Ogre::StringVectorPtr PakFileSystemArchive::find(const Ogre::String& pattern, bool recursive,bool dirs)
    {
		// directory change requires locking due to saved returns
		OGRE_LOCK_AUTO_MUTEX
    ...
    //-----------------------------------------------------------------------
    FileInfoListPtr PakFileSystemArchive::findFileInfo(const Ogre::String& pattern, 
	...
	//---------------------------------------------------------------------
	time_t PakFileSystemArchive::getModifiedTime(const String& filename)
	{
    ...
    //-----------------------------------------------------------------------
    const Ogre::String& PakFileSystemArchiveFactory::getType(void) const
    {
        static Ogre::String name = "FairyFileSystem";
        return name;
    }
