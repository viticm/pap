...
Terrain::_getPixmapCorner(const TerrainData::LayerInfo& layerInfo, TerrainData::Corner corner, Ogre::uint flags) const
{
	...
	return std::pair<Ogre::Real, Ogre::Real>((&pixmap.left)[indices[0]], (&pixmap.left)[indices[1]]);
}
...
Terrain::notifyHeightModified(int xstart, int zstart, int xend, int zend)
{
    ...
    if (!mData->mBoundBox.isNull())
    {
        ...
        for (int z = zstart; z < zend; ++z)
        {
            ...
            {
                Ogre::Real h = mData->getHeight(x, z);
                if (maximum.y < h)
                    maximum.y = h;
                else if (minimum.y > h)
                    minimum.y = h;
...
void
Terrain::notifyLightColourModified(int xstart, int zstart, int xend, int zend)
{
	...
	for (int z = ztile0; z < ztile1; ++z)
	{
		...
		{
			int xbais = x * tileSize;
			getTile(x, z)->_notifyLightColourModified(
				std::max(xstart - xbais, 0),
				std::max(zstart - zbais, 0),
				std::min(xend - xbais, tileSize + 1),
				std::min(zend - zbais, tileSize + 1));
...
void
Terrain::notifyColourModified(int xstart, int zstart, int xend, int zend)
{
	assert(0 <= xstart && xstart < xend && xend <= mData->mXSize+1);
	assert(0 <= zstart && zstart < zend && zend <= mData->mZSize+1);
	...
	for (int z = ztile0; z < ztile1; ++z)
	{
		...
		{
			int xbais = x * tileSize;
			getTile(x, z)->_notifyColourModified(
				std::max(xstart - xbais, 0),
				std::max(zstart - zbais, 0),
				std::min(xend - xbais, tileSize + 1),
				std::min(zend - zbais, tileSize + 1));
