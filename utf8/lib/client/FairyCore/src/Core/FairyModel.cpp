...
namespace Fairy {
    ...
    namespace
    {
        class ModelAnimationControllerValue : public Ogre::ControllerValue<Ogre::Real>
        {
        protected:
            Model* mModel;
    ...
    protected:
        Model* mModel;
       Ogre::String mModelResourceName;
       Ogre::String mModelResourceGroup;
        Ogre::Entity* mMasterEntity;
        Ogre::MovableObject* mLastMovable;
        Ogre::Node* mLastAttachNode;

    public:
        ModelSerializer(Model* model, const Ogre::String& resourceName, const Ogre::String& resourceGroup);
        ~ModelSerializer();

        void startElement(const Ogre::String& element, const XMLAttributes& attributes);
        void endElement(const Ogre::String& element);
    };

    //////////////////////////////////////////////////////////////////////////
    
    Model::Model(Ogre::SceneNode* parent, const Ogre::String& resourceName, const Ogre::String& resourceGroup)
        : mCreator()
        , mParent(parent)
        , mUserObject()
        , mSceneNodes()
    ...
    void Model::setUserObject(Ogre::UserDefinedObject* userObject)
    {
        ...
        for (ObjectList::const_iterator it = mMovables.begin(); it != mMovables.end(); ++it)
        {
			...
			movable->setUserAny(Ogre::Any(userObject));
        }
    }
    //-----------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------
    void Model::addTime(AnimationStateList& animationStates, Ogre::Real offset)
    {
    ...
    void Model::addTime(Ogre::Real offset)
    {
        addTime(mAnimationStates, offset);
    }
    //-----------------------------------------------------------------------
    void Model::create(const Ogre::String& resourceName, const Ogre::String& resourceGroup)
    {
        // HACK: Use expat parser now
    ...
    void Model::cleanup()
    {
		...
		//david-<<
		std::list<Ogre::SceneNode*>::iterator it = mSceneNodes.begin();
    ...
    //-----------------------------------------------------------------------
    ModelSerializer::ModelSerializer(Model* model, const Ogre::String& resourceName, const Ogre::String& resourceGroup)
        : mModel(model)
    ...
    //-----------------------------------------------------------------------
    void ModelSerializer::startElement(const Ogre::String& element, const XMLAttributes& attributes)
    {
        if (element == "entity")
        {
            ...
            // Get the mesh name
            const Ogre::String& meshName = attributes.getValue("mesh");
			...
			entity->setUserAny(Ogre::Any(mModel->mUserObject));
			//david-<<
            //if (normalise)
            //{
    ...
    //-----------------------------------------------------------------------
    void ModelSerializer::endElement(const Ogre::String& element)
    {
        if (element == "entity")
        {
            mLastMovable = NULL;
