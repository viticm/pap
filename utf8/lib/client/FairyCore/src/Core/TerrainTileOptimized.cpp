...
{
	static const Ogre::String type = "TerrainTileOptimized";
    return type;
}
...
//david-<<
void TerrainTileOptimized::visitRenderables(Ogre::Renderable::Visitor* visitor, 
...
TerrainTileOptimized::createGeometry(TerrainData* data, int xbase, int zbase, int xsize, int zsize)
{
    ...
    for (size_t layerIndex = 0; layerIndex < 2; ++layerIndex)
	{
        ...
        offset += 3 * sizeof(float);	
		// diffuse
		decl->addElement(MAIN_BINDING, offset, Ogre::VET_COLOUR, Ogre::VES_DIFFUSE);
		offset += sizeof(Ogre::VET_COLOUR);	
    ...
    for (MaterialBucketMap::const_iterator im = materialBucketMap.begin(); im != materialBucketMap.end(); ++im)
	{
        ...
        for (GridIdList::const_iterator igrid = mb->grids.begin(); igrid != mb->grids.end(); ++igrid)
		{
            ...
            for (size_t i = 0; i < 4; ++i)
			{
                ...
                *pFloat++ = v.x; *pFloat++ = v.y; *pFloat++ = v.z;	
				// diffuse
				Ogre::RGBA *pCol = static_cast<Ogre::RGBA*>(static_cast<void*>(pFloat));
				col = data->getDiffuseInfo((x+(corner&1)), (z+(corner>>1)));			
				Ogre::Root::getSingleton().convertColourValue(col, pCol++) ;
				pFloat =static_cast<float*>(static_cast<void*>(pCol));  			
                // layer 0
