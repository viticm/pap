...
const Ogre::LightList& TerrainTile::getLights(void) const
{
    ...
    // times we're asked
   Ogre::ulong frame = mNode->getCreator()->_getLightsDirtyCounter();
...
void
TerrainTile::_notifyLightColourModified(int xstart, int zbase, int xend, int zend)
{
	assert(0 <= xstart && xstart < xend && xend <= mXSize+1);
	assert(0 <= zbase && zbase < zend && zend <= mZSize+1);
	...
	OGRE_EXCEPT(Ogre::Exception::ERR_INTERNAL_ERROR,
		"The terrain is not editable",
		"TerrainTile::_notifyLightColourModified");
}

void
TerrainTile::_notifyColourModified(int xstart, int zbase, int xend, int zend)
{
	assert(0 <= xstart && xstart < xend && xend <= mXSize+1);
	assert(0 <= zbase && zbase < zend && zend <= mZSize+1);
