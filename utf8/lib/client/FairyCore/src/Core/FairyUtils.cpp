...
createSphere(const Ogre::String& name,
             const Ogre::Real radius, const int rings, const int segments,
             const bool normals,
             ...
             const bool inwards)
{
    ...
    Ogre::Real ringStep = 1 / (Ogre::Real) rings;
    Ogre::Real segmentStep = 1 / (Ogre::Real) segments;
    Ogre::Real ringAngleStep = Math::PI * ringStep;
    Ogre::Real segmentAngleStep = Math::TWO_PI * segmentStep;
    ushort vertexIndex = 0;

    for (int ring = 0; ring <= rings; ++ring)
    {
        Ogre::Real r0 = radius * Math::Sin(ring * ringAngleStep);
        Ogre::Real y0 = radius * Math::Cos(ring * ringAngleStep);

        // Generate the group of segments for the current ring
        for(int segment = 0; segment <= segments; ++segment)
        {
            Ogre::Real x0 = r0 * Math::Sin(segment * segmentAngleStep);
            Ogre::Real z0 = r0 * Math::Cos(segment * segmentAngleStep);
...
MeshPtr
createCommonSphere(const Ogre::Real radius, const int rings, const int segments,
                   const bool normals,
                   const int numTexCoordSets,
                   const bool inwards)
{
    static const Ogre::String DefaultNamePrefix("Fairy/Sphere");
...
MaterialPtr
createColourMaterial(const ColourValue& colour, const ColourValue& specular, Ogre::Real shininess)
{
...
MaterialPtr
createProjectiveMaterial(const Ogre::String& materialTemplateName, const Frustum* frustum)
{
...
AnimationState*
getAnimationStateSafely(AnimationStateSet* animationStateSet, const Ogre::String& name)
{
...
AnimationState*
getAnimationStateSafely(Entity* entity, const Ogre::String& name)
{
    assert(entity);
...
Animation*
getAnimationSafely(const Skeleton* skeleton, const Ogre::String& name)
{
    return skeleton ? skeleton->_getAnimationImpl(name) : 0;
}

Animation*
getAnimationSafely(const SkeletonPtr& skeleton, const Ogre::String& name)
{
    return getAnimationSafely(skeleton.get(), name);
}

Animation*
getAnimationSafely(const Mesh* mesh, const Ogre::String& name)
{
    return mesh ? getAnimationSafely(mesh->getSkeleton(), name) : 0;
}

Animation*
getAnimationSafely(const MeshPtr& mesh, const Ogre::String& name)
{
...
MeshPtr
createNullMeshForSkeleton(const Ogre::String& meshName, const SkeletonPtr& skeleton)
{
