...
const float MAX_DISTANCE_SQUARED = MAX_DISTANCE * MAX_DISTANCE;
    ...
    {
    public:
        IncrementalSceneBuilder(Fairy::System* system)
            : mSystem(system)
    ...
    //-----------------------------------------------------------------------
    void SceneInfo::load(XMLParser* parser, const Ogre::String& filename, const Ogre::String& groupName, bool completely)
    {
    ...
    //-----------------------------------------------------------------------
    void SceneInfo::save(XMLWriter* writer, const Ogre::String& filename)
    {
    ...
    //-----------------------------------------------------------------------
    void SceneInfo::initialise(Fairy::System* system)
    {
        assert(system);
        ...
        if (system->getDisableIncrementalBuildScene() || system->isEditable())
        {
            ...
            for (Objects::const_iterator it = mObjects.begin(); it != mObjects.end(); ++it)
            {
			    try
			    {
					...
					if(object->getCategory() == "Enviroment")
					{
						Fairy::EnviromentObject* pEnviromentObject = dynamic_cast<Fairy::EnviromentObject*>(object.get());
						if(pEnviromentObject)
						{
							pEnviromentObject->setSystem(system);
							pEnviromentObject->initialize();
						}
					}
                    if (object->hasProperty("create level"))
                    {
                        if ( false == system->_determineCreateLevel( 
                            VariantCast<Ogre::Real>( object->getProperty("create level") ) ) )
                            continue;
                    }
