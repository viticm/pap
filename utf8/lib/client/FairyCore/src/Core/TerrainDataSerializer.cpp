...
public:
	...
	*/
	virtual void startElement(const Ogre::String& element, const XMLAttributes& attrs);
...
{
    colour.r = attrs.getValueAs<Ogre::Real>("r");
    colour.g = attrs.getValueAs<Ogre::Real>("g");
    colour.b = attrs.getValueAs<Ogre::Real>("b");
}
...
void
TerrainData_xmlHandler::startElement(const Ogre::String& element, const XMLAttributes& attrs)
{
    if (element == "Terrain")
    {
    ...
    {
        mData->mScale = Ogre::Vector3(attrs.getValueAs<Ogre::Real>("x"),
                                      attrs.getValueAs<Ogre::Real>("y"),
                                      attrs.getValueAs<Ogre::Real>("z"));
    ...
    {
        mData->mScale = Ogre::Vector3(attrs.getValueAs<Ogre::Real>("x") / mData->mXSize,
                                      attrs.getValueAs<Ogre::Real>("y"),
                                      attrs.getValueAs<Ogre::Real>("z") / mData->mZSize);
    ...
    {
        mData->mPosition = Ogre::Vector3(attrs.getValueAs<Ogre::Real>("x"),
                                         attrs.getValueAs<Ogre::Real>("y"),
                                         attrs.getValueAs<Ogre::Real>("z"));
    }
    else if (element == "centre" || element == "center")
    {
        mData->mPosition = Ogre::Vector3(attrs.getValueAs<Ogre::Real>("x") - mData->mScale.x * mData->mXSize / 2,
                                         attrs.getValueAs<Ogre::Real>("y"),
                                         attrs.getValueAs<Ogre::Real>("z") - mData->mScale.z * mData->mZSize / 2);
    }
    else if (element == "heightmap")
    {
        const Ogre::String& type = attrs.getValueAs<String>("type", "standard");
        const Ogre::String& filename = attrs.getValue("filename");
    ...
    else if (element == "pixmap")
    {
        ...
        pixmap.left = attrs.getValueAs<Ogre::Real>("left", 0);
        pixmap.top = attrs.getValueAs<Ogre::Real>("top", 0);
        pixmap.right = attrs.getValueAs<Ogre::Real>("right", 1);
        pixmap.bottom = attrs.getValueAs<Ogre::Real>("bottom", 1);
    ...
    {
        const Ogre::String& type = attrs.getValueAs<String>("type", "image");
        const Ogre::String& filename = attrs.getValue("filename");
    ...
    {
        const Ogre::String& name = attrs.getValue("name");
        const Ogre::String& material = attrs.getValue("material");
    ...
    else if (element == "specular")
    {
        ...
        mData->mSurfaceParams.shininess = attrs.getValueAs<Ogre::Real>("shininess", mData->msDefaultSurfaceParams.shininess);
    }
...
void
TerrainData::_loadBaseInfo(XMLParser* parser, const Ogre::String& filename, const Ogre::String& groupName)
{
    assert(parser);
...
void
TerrainData::_saveBaseInfo(XMLWriter* writer, const Ogre::String& filename) const
{
    assert(writer);
        ...
        mSurfaceParams != msDefaultSurfaceParams)
    {
        ...
        for (MaterialTemplates::const_iterator it = mMaterialTemplates.begin(); it != mMaterialTemplates.end(); ++it)
        {
            ...
            const Ogre::String& material = it->second;
...
TerrainData::_fixupMissingInfo(void)
{
	...
	if (mWaterFogReplacements.empty())
	{
		FogReplacement fogReplacement;

		fogReplacement.programNames[Ogre::FOG_NONE] = "Water/Colour_Middle_ps";
		fogReplacement.programNames[Ogre::FOG_EXP] = "Water/Colour_Middle_ps%fog_exp";
		fogReplacement.programNames[Ogre::FOG_EXP2] = "Water/Colour_Middle_ps%fog_exp2";
		fogReplacement.programNames[Ogre::FOG_LINEAR] = "Water/Colour_Middle_ps%fog_linear";
		mWaterFogReplacements.push_back(fogReplacement);
	}
}
