...
TerrainData::reset(void)
{
	...
	mWaterFogReplacements.clear();
	mWaterSupportedFogReplacements.clear();
}
...
TerrainData::_computeTileBoundingBox(Ogre::AxisAlignedBox& box, int startx, int startz, int width, int depth) const
{
	...
	Ogre::Real minHeight = Ogre::Math::POS_INFINITY;
	Ogre::Real maxHeight = Ogre::Math::NEG_INFINITY;
	size_t patch = mXSize + 1;
	const Ogre::Real* pHeight = &mHeightmap[startz * patch + startx];
    for (int z = 0; z <= depth; ++z, pHeight += patch)
    {
        for (int x = 0; x <= width; ++x)
        {
            Ogre::Real h = pHeight[x];
...
const uint32 TERRAIN_GRIDINFO_VERSION = 0x00100003;

void
TerrainData::_loadHeightmap(const Ogre::String& filename, const Ogre::String& type, const Ogre::String& groupName)
{
    // Allocate buffers
    ...
    else if (type == "image")
    {
        ...
        if (is16bit)
        {
            ...
            for (size_t j = 0; j <= depth; ++j)
            {
                ...
                {
                    *pDest++ = Ogre::Real(*pSrc16++) * invScale;
                }
                if (flipTerrain)
                {
                    // Work backwards
        ...
        else
        {
            ...
            for (size_t j = 0; j <= depth; ++j)
            {
                for (size_t i = 0; i <= width; ++i)
                {
                    *pDest++ = Ogre::Real(*pSrc8++) * invScale;
                }
                if (flipTerrain)
                {
                    // Work backwards
...
void
TerrainData::_saveHeightmap(const Ogre::String& filename) const
{
...
void
TerrainData::_loadLightmap(const Ogre::String& filename, const Ogre::String& type, const Ogre::String& groupName)
{
...
void
TerrainData::_saveLightmap(const Ogre::String& filename) const
{
    if (!mLightmapImage)
    {
        return;
...
void
TerrainData::_loadGridInfo(const Ogre::String& filename, const Ogre::String& groupName)
{
    ...
    //{
        ...
        else
        {
            ...
            for (GridInfoArray::iterator it = mGridInfos.begin(); it != mGridInfos.end(); ++it)
            {
				...
				if (header[1] >= 0x00100003)
				{
					bool hasDiffuse = false;
					reader.readBools(&hasDiffuse);
					if (hasDiffuse)
					{
						uchar col[3];
						reader.readData(&col, sizeof col);

						Ogre::ColourValue color = Ogre::ColourValue::White;
						color.r = (float) col[0]/256.0f;
						color.g = (float) col[1]/256.0f;
						color.b = (float) col[2]/256.0f;

						Point point;
						point.x = gridIndex % mXSize;
						point.y = gridIndex / mXSize;

						mDiffuseInfoMap.insert(DiffuseInfoMap::value_type(point, color));
						mLightDiffuseInfoMap.insert(DiffuseInfoMap::value_type(point,  Ogre::ColourValue::Black));
...
void
TerrainData::_saveGridInfo(const Ogre::String& filename) const
{
    FileWriter writer(filename);
    ...
    for (GridInfoArray::const_iterator it = mGridInfos.begin(); it != mGridInfos.end(); ++it)
    {
		...
		// Terrain Diffuse Info
		Point point;
		point.x = gridIndex % mXSize;
		point.y = gridIndex / mXSize;
		DiffuseInfoMap::const_iterator it = mDiffuseInfoMap.find(point);
		bool hasDiffuse = false;
		if (it != mDiffuseInfoMap.end())
		{
			// Save diffuse info
			hasDiffuse = true;
			writer.writeBools(&hasDiffuse);

			Ogre::ColourValue color = it->second;

			uchar col[3];			
			col[0] = (color.r*256>255)?255: (color.r*256);
			col[1] = (color.g*256>255)?255: (color.g*256);
			col[2] = (color.b*256>255)?255: (color.b*256);
			writer.writeData(&col, sizeof col);
		}
		else
		{
			 writer.writeBools(&hasDiffuse);
...
bool 
TerrainData::_findGridInfoCopy(const Ogre::String& fileName, size_t& index)
{
...
TerrainData::_fixupSupportedMaterials(void)
{
	...
	for (FogReplacements::const_iterator it = mWaterFogReplacements.begin(); it != mWaterFogReplacements.end(); ++it)
	{
		const FogReplacement& fogReplacement = *it;
		bool supported = true;
		for (size_t i = 0; i < sizeof(fogReplacement.programNames)/sizeof(*fogReplacement.programNames); ++i)
		...
		if (supported)
		{
			mWaterSupportedFogReplacements.push_back(fogReplacement);
		}
	}
}

void
TerrainData::load(XMLParser* parser, const Ogre::String& filename, const Ogre::String& groupName, bool loadLightmap)
{
    // Clear old data
    reset();
...
void
TerrainData::save(XMLWriter* writer, const Ogre::String& filename, bool preservingNames) const
{
    assert(writer);
...
TerrainData::getBoundBox() const
{
    if (mBoundBox.isNull())
    {
        if (!mHeightmap.empty())
        {
            ...
            Ogre::Real minHeight = *it;
            Ogre::Real maxHeight = *it;
...
std::pair<int, int>
TerrainData::getJunction(Ogre::Real x, Ogre::Real z) const
{
    return std::pair<int, int>(
        fast_ifloor((x - mPosition.x) * mInvScale.x + (Ogre::Real)0.5),
        fast_ifloor((z - mPosition.z) * mInvScale.z + (Ogre::Real)0.5));
}

std::pair<Ogre::Real, Ogre::Real>
TerrainData::worldToJunction(Ogre::Real x, Ogre::Real z) const
{
    return std::pair<Ogre::Real, Ogre::Real>(
        ((x - mPosition.x) * mInvScale.x + (Ogre::Real)0.5),
        ((z - mPosition.z) * mInvScale.z + (Ogre::Real)0.5));
}

std::pair<Ogre::Real, Ogre::Real>
TerrainData::junctionToWorld(Ogre::Real x, Ogre::Real z) const
{
    return std::pair<Ogre::Real, Ogre::Real>(
        ((x - (Ogre::Real)0.5) * mScale.x + mPosition.x),
        ((z - (Ogre::Real)0.5) * mScale.z + mPosition.z));
}

std::pair<int, int>
TerrainData::getGrid(Ogre::Real x, Ogre::Real z) const
{
    return std::pair<int, int>(
...
std::pair<Ogre::Real, Ogre::Real>
TerrainData::worldToGrid(Ogre::Real x, Ogre::Real z) const
{
    return std::pair<Ogre::Real, Ogre::Real>(
...
std::pair<Ogre::Real, Ogre::Real>
TerrainData::gridToWorld(Ogre::Real x, Ogre::Real z) const
{
    return std::pair<Ogre::Real, Ogre::Real>(
...
Ogre::Real
TerrainData::getHeightAt(Ogre::Real x, Ogre::Real z) const
{
    if (mXSize <= 0 || mZSize <= 0)
        return 0;

    Ogre::Real x_pct, z_pct;
    int x_index, z_index;
...
Ogre::Real 
TerrainData::getHeightAtGrid(Ogre::Real x, Ogre::Real z) const
{
    std::pair<Ogre::Real, Ogre::Real> world = gridToWorld(x, z);
	return getHeightAt(world.first, world.second);
}

Ogre::Vector3
TerrainData::getPositionAt(Ogre::Real x, Ogre::Real z) const
{
    return Ogre::Vector3(x, getHeightAt(x, z), z);
}

Ogre::Vector3
TerrainData::getPositionAtGrid(Ogre::Real x, Ogre::Real z) const
{
    std::pair<Ogre::Real, Ogre::Real> world = gridToWorld(x, z);
    return Ogre::Vector3(
        world.first,
...
Ogre::Vector3
TerrainData::getNormalAt(Ogre::Real x, Ogre::Real z) const
{
...
void
TerrainData::getPositionAndNormalAt(Ogre::Real x, Ogre::Real z, Ogre::Vector3& position, Ogre::Vector3& normal) const
{
...
{
    return Ogre::Vector3(mPosition.x + (x + (Ogre::Real)0.5) * mScale.x,
                         getGridHeight(x, z),
                         mPosition.z + (z + (Ogre::Real)0.5) * mScale.z);
}

std::pair<bool, Ogre::Real>
...
namespace {
    struct IsCaseInsensitiveEqual
    {
        ...
        IsCaseInsensitiveEqual(const Ogre::String& value) : value(value) {}
    ...
    {
        bool operator() (const Ogre::String& lhs, const Ogre::String& rhs) const
        {
...
int
TerrainData::_registerPixmap(const Ogre::String& textureName, Ogre::Real left, Ogre::Real top, Ogre::Real right, Ogre::Real bottom)
{
    size_t pixmapId;
   Ogre::String texture = textureName;
...
TerrainData::_optimize(void)
{
...

    {
        ...
        for (size_t oldTextureId = 0; oldTextureId < mTextures.size(); ++oldTextureId)
        {
            ...
            {
               Ogre::String texture = mTextures[oldTextureId];
        ...
        {
            const Ogre::String& texture = it->first;
                           ...
                           bool positiveSide, bool negativeSide) const
{
    ...
    else
    {
        ...
        std::pair<bool ,Ogre::Real> r(false, 0);

        Ogre::Real xstep = ray.getDirection().x * mScale.z;
        Ogre::Real zstep = ray.getDirection().z * mScale.x;
        if (Ogre::Math::Abs(xstep) < Ogre::Math::Abs(zstep))
        {
            // step base z axis
            int zinc = ray.getDirection().z > 0 ? +1 : -1;
            Ogre::Real zstart = (start.z - mPosition.z) * mInvScale.z;
            int zbegin = fast_ifloor(zstart) - zinc;
            int zend = fast_ifloor((end.z - mPosition.z) * mInvScale.z) + zinc;
            Ogre::Real xfactor = xstep / zstep;
            Ogre::Real xinc = zinc * xfactor;
            Ogre::Real xiter = ((start.x - mPosition.x) * mInvScale.x) - (zstart - zbegin) * xfactor;
        ...
        else
        {
            ...
            Ogre::Real xstart = (start.x - mPosition.x) * mInvScale.x;
            int xbegin = fast_ifloor(xstart) - xinc;
            int xend = fast_ifloor((end.x - mPosition.x) * mInvScale.x) + xinc;
            Ogre::Real zfactor = zstep / xstep;
            Ogre::Real zinc = xinc * zfactor;
            Ogre::Real ziter = ((start.z - mPosition.z) * mInvScale.z) - (xstart - xbegin) * zfactor;
                                    ...
                                    std::vector<Ogre::Vector3>& positions, std::vector<Ogre::Vector3>* normals)
{
    ...
    const Ogre::Real minY = bound.getMinimum().y;
    const Ogre::Real maxY = bound.getMaximum().y;
    Ogre::Real minX = Ogre::Math::POS_INFINITY;
    Ogre::Real minZ = Ogre::Math::POS_INFINITY;
    Ogre::Real maxX = Ogre::Math::NEG_INFINITY;
    Ogre::Real maxZ = Ogre::Math::NEG_INFINITY;
    const Ogre::Real epsilon = 1e-3f;
    for (size_t e = 0; e < sizeof(edges)/sizeof(*edges); ++e)
    {
        ...
        {
            Ogre::Real denom = 1 / diff;
            Ogre::Real t;

            t = (minY - v0.y) * denom;
            if (0 <= t && t <= 1)
            {
                Ogre::Real x = v0.x + (v1.x - v0.x) * t;
                Ogre::Real z = v0.z + (v1.z - v0.z) * t;
                if (minX > x) minX = x;
                if (maxX < x) maxX = x;
                if (minZ > z) minZ = z;
                if (maxZ < z) maxZ = z;
    ...
    if (sizeX*sizeZ > areaLimit)
    {
        ...
        if (sizeX < sizeZ)
        {
            sizeX = std::max(fast_ifloor(Ogre::Math::Sqrt(
                (Ogre::Real)areaLimit * sizeX / sizeZ)), 1);
            sizeZ = areaLimit / sizeX;
        }
        else
        {
            sizeZ = std::max(fast_ifloor(Ogre::Math::Sqrt(
                (Ogre::Real)areaLimit * sizeZ / sizeX)), 1);
...
void TerrainData::setDiffuseInfo(int x, int z, const Ogre::ColourValue& color)
{
	if (0 > x || x > mXSize || 0 > z && z > mZSize)
	{
		return;
	}

	Ogre::ColourValue deltaWhite = Ogre::ColourValue::White-color;
	Point pt(x,z);
	if (mDiffuseInfoMap.find(pt) == mDiffuseInfoMap.end())
	{
		if (deltaWhite.r<0.02 && deltaWhite.g<0.02 && deltaWhite.b<0.02)
		{
			return;
		}
		Ogre::ColourValue col(color);
		mDiffuseInfoMap.insert(DiffuseInfoMap::value_type(pt, col));
	}
	else
	{
		DiffuseInfoMap::iterator iter = mDiffuseInfoMap.find(pt);
		if (deltaWhite.r<0.02 && deltaWhite.g<0.02 && deltaWhite.b<0.02)
		{
			mDiffuseInfoMap.erase(iter);
		}
		else
		{
			Ogre::ColourValue& oldColour = iter->second;
			oldColour = color;
		}		
	}
}

void TerrainData::setLightDiffuseInfo(int x, int z, const Ogre::ColourValue& color)
{
	if (0 > x || x > mXSize || 0 > z && z > mZSize)
	{
		return;
	...
	Ogre::ColourValue deltaWhite = Ogre::ColourValue::White-color;
	Point pt(x,z);
	if (mLightDiffuseInfoMap.find(pt) == mLightDiffuseInfoMap.end())
	{

		Ogre::ColourValue col(color);
		mLightDiffuseInfoMap.insert(DiffuseInfoMap::value_type(pt, col));
	}
	else
	{
		DiffuseInfoMap::iterator iter = mLightDiffuseInfoMap.find(pt);
			Ogre::ColourValue& oldColour = iter->second;
			oldColour += color;
...
Ogre::ColourValue TerrainData::getDiffuseInfo(int x, int z)
{
	Point pt(x,z);
	if (mDiffuseInfoMap.find(pt) == mDiffuseInfoMap.end())
	{
...
Ogre::ColourValue TerrainData::getLightDiffuseInfo(int x, int z)
{
	Point pt(x,z);
	if (mLightDiffuseInfoMap.find(pt) == mLightDiffuseInfoMap.end())
	{
