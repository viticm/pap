...
protected:
    struct MaterialId
    {
        ...
        MaterialId(uchar textureId0, uchar textureId1,Ogre::ulong lightmapId, ushort depthBias)
...
//laim----	System*	mSystem;
    ...
    Ogre::Real mAtlasBlockTexCoordInc;    //!< = 1.0f / mAtlasBlockSize
...
public:
    ...
    std::pair<Ogre::Real, Ogre::Real> _getPixmapCorner(const TerrainData::LayerInfo& layerInfo, TerrainData::Corner corner, Ogre::uint flags) const;
...
/// enable title query by other class for modify some params
	typedef Ogre::VectorIterator< TileList >	TileListIterator;
	TileListIterator	getTileIterator()	{return TileListIterator(mTiles);}
public:
    Terrain(void);
    ~Terrain();
    ...
    */
    ...
    Ogre::Real getHeightAt(float x, float z) const;
	...
	void notifyColourModified(int xstart, int zstart, int xend, int zend);
	void notifyLightColourModified(int xstart, int zstart, int xend, int zend);
};
