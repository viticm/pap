...
#include <OgreAnimation.h>			//add by zhuwenchao 2010.1.5
#include <OgreSkeleton.h>			//add by zhuwenchao 2010.1.8  for get <BoneIterator>
...
DObject_NT::DObject_NT( Fairy::System *system )
: m_pObjNode(NULL)
...
DObject_NT::~DObject_NT()
{
	...
	if (m_pObjNode)
	{
		m_pObjNode->getCreator()->destroySceneNode(m_pObjNode);
		m_pObjNode = NULL;
	}
}
...
void DObject_NT::SetObjName(DString StrObjName)
{
		...
		{			
			...
			{			
				...
				if ( mCurrentSkill )
				{
					...
					{
						Fairy::AnimationEffectInfo *effectInfo = mCurrentSkill->getAnimationEffectInfo(i);
						assert (effectInfo);
							...
							(currtime < oldtime && (time >= oldtime || time < currtime)))
						{
							...
							if (effect)
							{
								if ( !m_bAnimationLoop )
								{
									Fairy::EffectManager::getSingleton().removeEffect(effect, false);
									effect = NULL;
								}
							}
							else
							{
								effect = Fairy::EffectManager::getSingleton().createEffect(effectInfo->getEffectTemplateName());
					...
					{
						Fairy::AnimationRibbon *ribbon = mCurrentSkill->getAnimationRibbon(i);
						assert (ribbon);
					...
					if (m_fOnPlaySound)
					{
						...
						{					
							Fairy::AnimationSound *sound = mCurrentSkill->getAnimationSound(i);
							assert (sound);
							...
							if (false == sound->mPlayed)
							{
								...
								{
									//wangling-<<
									const Ogre::Matrix4& mtx = m_pObjNode->_getFullTransform();
...
bool DObject_NT::_AddAttrib(const DString& name, const DString& value, const DString& hints)
{
	...
	for(long i = 0; i < slen; i ++)
	{
		...
		else if(pTemp[i] == 'T') av.Hints |= AH_MATERIAL; //add by zhuwenchao 2010.1.21 加一个材属性
	}
	av.Value = value;
	av.IsChanged = true;
...
bool DObject_NT::_SetAttrib(const DString& name, const DString& value, const DString& hints)
{
	...
	for(long i = 0; i < slen; i ++)
	{
		...
		else if(pTemp[i] == 'T') it->second.Hints |= AH_MATERIAL; //add by zhuwenchao 2010.1.21 加一个材属性
	}
...
{
	DString sh = "";
	if(hints & AH_MESH) sh += /*"M"*/"Mesh";
	else if(hints & AH_ANIMATION) sh += /*"A"*/"Animation";
	else if(hints & AH_LOCATOR) sh += /*"L"*/"SlotCreate";
	else if(hints & AH_MATERIAL) sh += "Material"; //add by zhuwenchao 2010.1.21 加一个材属性
	return sh;
}
...
dword DObject_NT::_Str2Hint(const DString& hints)
{
	...
	for(long i = 0; i < slen; i ++)
	{
		...
		else if(pTemp[i] == 'T') ret |= AH_MATERIAL; //add by zhuwenchao 2010.1.21 加一个材属性
	}
	return ret;
}
...
bool DObject_NT::_LoadEntity()
{
	...
	for(it = m_EntityList.begin(); it != m_EntityList.end(); it ++)
	{
		try
		{
            ...
            Ogre::MeshPtr mesh = Fairy::loadMesh(it->File, m_GroupName, m_ObjFile, m_GroupName);
            if (!mesh.isNull())
            {
...
bool DObject_NT::_GetLocatorWorldTransform(const DString& name, Ogre::Matrix4& mtx)
{
	...
	for(it = m_LocatorList.begin(); it != m_LocatorList.end(); it ++)
	{
		if(it->Name == name)
		{
			...
			{
				//wangling-<<
				mtx = it->pLocatorNode->_getFullTransform();
				return true;
				//wangling->>
			}
			else
...
bool DObject_NT::_GetLocatorWorldTransform(const DString& name, Fairy::TransformInfo &info, const Ogre::Vector3 &offsetPos,
...
bool DObject_NT::_GetBoneWorldTransform(const DString& name, Fairy::TransformInfo &info, const Ogre::Vector3 &offsetPos,
										const Ogre::Quaternion &offsetRotation, GetTransformInfoType type )
{
...
bool	DObject_NT::_UpdateLocatorPos()
{
	...
	for(it = m_LocatorList.begin(); it != m_LocatorList.end(); it ++)
	{
		if(m_pObjSkeleton && it->BoneName.size() > 0)
		{
			...
			//wangling-<<
			worldpos = bone->_getDerivedPosition();
			worldrot = bone->_getDerivedOrientation();
			//wangling->>
			if(it->pLocatorNode)
			{
...
bool DObject_NT::_DestroyLocators()
{
	...
	for(it = m_LocatorList.begin(); it != m_LocatorList.end(); it ++)
	{
		...
		if(it->pLocatorNode)
		{
			Ogre::SceneManager* pSceneManger =  it->pLocatorNode->getCreator();
			pSceneManger->destroySceneNode(it->pLocatorNode);
...
void DObject_NT::CreateSkill(const DString &skillName, bool loop, bool anim, Ogre::Real delayTime)
{
	...
	if ( false == anim )
	{
		mCurrentSkill = Fairy::EffectManager::getSingleton().createSkill(skillName);
...
void DObject_NT::DelCurrentSkill(void)
{
	if (mCurrentSkill)
	{
		...
		Fairy::EffectManager::getSingleton().removeSkill(mCurrentSkill);
		mCurrentSkill = NULL;
	}
}
...
Ogre::Entity * DObject_NT::_LoadEntity( const DString& meshName )
{
	...
	try
	{
        ...
        Ogre::MeshPtr mesh = Fairy::loadMesh(meshName, m_GroupName, m_ObjFile, m_GroupName);
        if (!mesh.isNull())
        {
...
{	
	...
	Fairy::Effect *effect = Fairy::EffectManager::getSingleton().createEffect(effectName);
...
std::pair<bool, FairyEffectMap::iterator> DObject_NT::DelEffect( EffectHandle handle )
{
	...
	{
		Fairy::EffectManager::getSingleton().removeEffect((i->second).mEffect);
...
void DObject_NT::_updateFairyEffects(void)
{
	...
	{
		Fairy::Effect *effect = (i->second).mEffect;
...
void DObject_NT::UpdateAnimationList(DString meshname)
{
	if(true == meshname.empty())
		return;
	if(NULL == m_pObjFirstSkeletonEntity)
		return;
	int nAniNum = static_cast<int>(m_pObjFirstSkeletonEntity->getSkeleton()->getNumAnimations());
	for(int i=0;i<nAniNum;i++)
	{
		DString AnimName = m_pObjFirstSkeletonEntity->getSkeleton()->getAnimation(i)->getName();
		m_AniList.push_back(AnimName);
	}
	DStringList plist = m_AniList;
	m_DefaultSkeletonName = m_pObjFirstSkeletonEntity->getSkeleton()->getName();
	m_LinkMap.insert(AnimationSkeletonLinksMap::value_type(m_DefaultSkeletonName,plist));
	//更新骨骼与骨架关联表
	UpdateSkelBoneMap(m_pObjFirstSkeletonEntity->getSkeleton()->getName());
}

void DObject_NT::UpdateLinkSkeAnimationList(DStringList inifile)
{
	if(NULL == m_pObjFirstSkeletonEntity)
		return;
	if(inifile.empty())
		return;
	int filecount = static_cast<int>(inifile.size());
	bool bcorrectdata = false;
	//获取当前实体关联的骨骼名
	DString defaultskelname = m_pObjFirstSkeletonEntity->getSkeleton()->getName();
	for(int i=0;i<filecount;++i)
	{
		//取出骨骼列表中文骨骼文件名
		Ogre::DataStreamPtr stream = Ogre::ResourceGroupManager::getSingleton().openResource(inifile[i], m_GroupName);
		//先比较一下是否为相同结构的骨骼，不然会死人的...
		while(!stream->eof())
		{
			DString line;
			line = stream->getLine();
			//与文件中有相同骨骼名，说明是相同结构的骨骼，为正确的数据
			if(0 == line.compare(defaultskelname))
			{
				bcorrectdata = true;
				continue;
			}
			m_SkeFileNameVector.push_back(line);
		}
		//若当前文件里没有相同骨骼名，就清空列表,有的话就跳出，一个骨骼只对应一个表
		if(!bcorrectdata)
			m_SkeFileNameVector.clear();
		else
			break;
	}
	//如果不是就清除共享的骨骼名，只添加默认的骨骼名就可以了
	if(!bcorrectdata)
	{
		m_SkeFileNameVector.push_back(defaultskelname);
		return;
	}
	int nNum = static_cast<int>(m_SkeFileNameVector.size());

	//获取原骨骼实例，LINK上新的骨骼，并更新动画
	Ogre::SkeletonInstance* oldSkel = m_pObjFirstSkeletonEntity->getSkeleton();
	for(int i=0;i<nNum;i++)
	{			
		oldSkel->addLinkedSkeletonAnimationSource(m_SkeFileNameVector[i]);
		//更新骨骼与骨架关联表
		UpdateSkelBoneMap(m_SkeFileNameVector[i]);
	}
	m_pObjFirstSkeletonEntity->refreshAvailableAnimationState();

	//获取LINK骨骼的迭带器，更新动画列表
	Ogre::Skeleton::LinkedSkeletonAnimSourceIterator Linkite = oldSkel->getLinkedSkeletonAnimationSourceIterator();	
	int Index = 0;
	while(Linkite.hasMoreElements())
	{
		Ogre::SkeletonPtr pSkeleton = Linkite.getNext().pSkeleton;
		unsigned short num = pSkeleton->getNumAnimations();
		DStringList plist;		//用于获取单个骨骼上的动画列表
		for(int i=0;i<num;i++)
		{
			DString AnimName = pSkeleton->getAnimation(i)->getName();
			m_AniList.push_back(AnimName);
			plist.push_back(AnimName);
		}
		m_LinkMap.insert(AnimationSkeletonLinksMap::value_type(m_SkeFileNameVector[Index],plist));
		Index++;
	}
}

DStringList& DObject_NT::GetObjAllAnimNameEx()
{
    return m_AniList;
}

void DObject_NT::UpdateSkelBoneMap(DString Skelname)
{
	if(Skelname.empty())
		return;
	if(NULL == m_pObjFirstSkeletonEntity)
	...
	SkelBoneLinks::const_iterator its;
	if(!SkelBoneLinksmap.empty())
	{ 
		its = SkelBoneLinksmap.find(Skelname);
		if(its != SkelBoneLinksmap.end())
			return;
	}
	Ogre::Skeleton::BoneIterator itb = m_pObjFirstSkeletonEntity->getSkeleton()->getBoneIterator();
	DString bonename;
	DStringList pbonelist;
	while(itb.hasMoreElements())
	{
		Ogre::Bone* bone = itb.getNext();
		if(NULL == bone)
			continue; 
		bonename = bone->getName();
		pbonelist.push_back(bonename);
	}
	SkelBoneLinksmap.insert(SkelBoneLinks::value_type(Skelname,pbonelist));

	m_BoneList = pbonelist;
}

bool DObject_NT::_UpdateLocatorPos(const DString& locatorname,const Ogre::Vector3& position,const Ogre::Quaternion& quaternion)
{
	...
	for(it = m_LocatorList.begin(); it != m_LocatorList.end(); it ++)
	{
		if(locatorname.compare(it->Name) == 0)
		{
			Ogre::Bone* bone = m_pObjSkeleton->getBone(it->BoneName);
			if(!bone)
			{
			...
			LocatorOffRot.FromAngleAxis(Ogre::Radian(quaternion.w), Ogre::Vector3(quaternion.x, quaternion.y, quaternion.z));
