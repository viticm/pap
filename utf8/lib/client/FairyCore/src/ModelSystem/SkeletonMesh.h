...
#define AH_MATERIAL						(0x08)				//add by zhuwenchao 2010.1.21 加一个材属性
...
public:
	...
	void UpdateAnimationList(DString meshname);					// 更新指定DObject的动画列表
	void UpdateLinkSkeAnimationList(DStringList inifile);		// 更新共享骨骼的动画列表
	DStringList& GetObjAllAnimNameEx();							// 获取动画名称列表(若有共享骨骼则用这个)
	void UpdateSkelBoneMap(DString Skelname);					// 更新骨骼与骨头关联表
	//=====================================================================================================}
...
public:
	//{=====================================================================================================
	//add by zhuwenchao 2010.1.5
	DStringList	m_SkeFileNameVector;													//骨骼文件名字列表

	typedef DStringList AnimationListIterator;						
	AnimationListIterator m_AniList;													//动画总列表(一个实体的所有动作)

	typedef std::map<DString,AnimationListIterator> AnimationSkeletonLinksMap;
	AnimationSkeletonLinksMap m_LinkMap;												//骨骼与动画关联表

	typedef DStringList BoneListIterator;												//骨头文件名字列表	
	BoneListIterator m_BoneList;

	typedef std::map<DString,BoneListIterator> SkelBoneLinks;							//骨骼与骨头关联表
...
protected://functions
	...
	bool							_UpdateLocatorPos(const DString& locatorname,const Ogre::Vector3& position,const Ogre::Quaternion& quaternion);	//add by zhuwenchao 2010.1.12 增加一个更新挂点位置的函数
