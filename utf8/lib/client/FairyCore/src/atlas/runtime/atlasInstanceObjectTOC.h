...
#ifndef _ATLASINSTANCEOBJECTTOC_H_
#define _ATLASINSTANCEOBJECTTOC_H_
...
class AtlasInstanceObjectStub : public AtlasInstanceStub<AtlasObjectChunk, AtlasInstanceObjectStub>
{
public:
   AtlasInstanceObjectStub()
   {
      mBounds.minExtents.set(0,0,0);
      mBounds.maxExtents.set(0,0,0);
   }
...
/// 
/// @ingroup AtlasRuntime
class AtlasInstanceObjectTOC : public AtlasInstanceTOC<AtlasInstanceObjectStub, AtlasResourceObjectTOC>
{
   typedef AtlasInstanceTOC<AtlasInstanceObjectStub, AtlasResourceObjectTOC> Parent;
   ...
   void updateBounds(AtlasInstanceObjectStub *stub, U32 i, S32 x, S32 y)
   {
      // This seems silly...
      ...
      for(S32 j=0; j<4; j++)
      {
         AssertWarn(stub->mBounds.isContained(stub->mChildren[j]->mBounds),
            avar("AtlasInstanceObjectTOC::updateBounds - invalid child bounds! %d@(%d, %d)", i, x, y));
      }
   }
...
public:
   ...
   virtual void requestLoad(AtlasInstanceObjectStub *stub, U32 reason, F32 priority);
   virtual void cancelLoadRequest(AtlasInstanceObjectStub *stub, U32 reason);
   ...
   bool castRay(const Point3F &start, const Point3F &end, RayInfo* info,StubType** stub,bool emptyCollide);
