...
void AtlasInstanceObjectTOC::requestLoad(AtlasInstanceObjectStub *stub, U32 reason, F32 priority)
{
...
void AtlasInstanceObjectTOC::cancelLoadRequest(AtlasInstanceObjectStub *stub, U32 reason)
{
   // Skip load requests for leafs if we've already paged in all the leaves.
   if(mLeafCollisionMode && stub->level == mTreeDepth - 1)
      return;
...
bool AtlasInstanceObjectTOC::attemptSplit(StubType *s)
{
   // Try some early outs.
...
void AtlasInstanceObjectTOC::doSplit(StubType *s)
{
   if(s->isSplit())
      return;
...
void AtlasInstanceObjectTOC::warmUpData(StubType *s, F32 priority)
{
...
void AtlasInstanceObjectTOC::processLOD(SceneState *state)
{
   PROFILE_START(AtlasInstanceObjectTOC_processLOD);
...
void AtlasInstanceObjectTOC::renderBounds(const S32 debugMode)
{
   PROFILE_START(AtlasInstanceObjectTOC_renderBounds);
   ...
   while(stackDepth)
   {
      ...
      ColorI boxColor;
      AtlasResourceObjectStub *args = getResourceStub(s);
      ...
      default:
         AssertISV(false, "AtlasInstanceObjectTOC::renderBounds - unknown debug mode!");
         break;
      }
...
void AtlasInstanceObjectTOC::batchGeometry(AtlasClipMapBatcher *cmb)
{
   AssertFatal(cmb, "AtlasInstanceObjectTOC::renderGeometry - I need an AtlasClipMapBatcher!");
   PROFILE_START(AtlasInstanceObjectTOC_batchGeometry);
...
void AtlasInstanceObjectTOC::clear(const F32 deltaT)
{
#if 0
   // We need a stack...
   FrameAllocatorMarker fm;
...
bool AtlasInstanceObjectTOC::castRay(const Point3F &start, const Point3F &end, RayInfo* info,StubType** stub,bool emptyCollide)
{
      ...
      if(s->hasChildren())
      {
         for(S32 i=0; i<4; i++)
         {
            ...
            AssertFatal(s->mBounds.isContained(s->mChildren[i]->mBounds), 
               "AtlasInstanceObjectTOC::castRay - child not contained by parent!");
            stack[stackDepth++] = s->mChildren[i];
         }
...
bool AtlasInstanceObjectTOC::buildCollisionInfo(const Box3F &box, Convex *c, 
                                              AbstractPolyList *poly, AtlasInstance *object)
{
   ...
   while(stackDepth)
   {
      ...
      AtlasResourceObjectStub *rs = getResourceStub(s);
      AssertFatal(rs->mChunk, "AtlasResourceObjectTOC::buildCollisionInfo - chunk not present for collision query!");
      if(rs->mChunk)
      {
      ...
      {
         Con::errorf("AtlasResourceObjectTOC::buildCollisionInfo - chunk not present for collision query (%d,%d @ %d)!", s->pos.x, s->pos.y, s->level);
      }
   }
...
void AtlasInstanceObjectTOC::loadCollisionLeafChunks()
{
   AssertISV(!mLeafCollisionMode, "AtlasInstanceObjectTOC::loadCollisionLeafChunks - already in collision mode!");

   Con::printf("AtlasInstanceObjectTOC::loadCollisionLeafChunks - Loading leaf collision chunks...");

   for(S32 x=0; x<BIT(mTreeDepth - 1); x++)
   {
      for(S32 y=0; y<BIT(mTreeDepth - 1); y++)
      {
         AtlasInstanceObjectStub *aigs = getStub(mTreeDepth-1, Point2I(x,y));
         AtlasResourceObjectStub *args = getResourceStub(aigs);
   ...
   Con::printf("AtlasInstanceObjectTOC::loadCollisionLeafChunks - DONE.");
}