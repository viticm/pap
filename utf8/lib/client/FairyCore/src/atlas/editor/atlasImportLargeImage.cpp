...
private:
   ...
   static int jpegErrorFn(void *client_data)
   {
      ...
      return (stream->mFile->getStatus() != Torque::FS::FileNode::Open &&
         stream->mFile->getStatus() != Torque::FS::FileNode::Closed);
   }
...
public:
   ...
   virtual bool open(const char *image)
   {
      ...
      mStartOfFile = true;
      mFile = Torque::FS::OpenFile(image, Torque::FS::File::Read);
                ...
                "to store this data.")
{
   ...
   for(S32 i=0; i<tileCount; i++)
   {
      ...
      for(S32 j=0; j<tileCount; j++)
      {
         ...
         constructInPlace< GBitmap >( gb, tileSize, tileSize );
         for(S32 k=0; k<tileSize; k++)
