...
#define wxPG_USE_USERDATA               1

// 1 to allow per-property validators
#define wxPG_USE_VALIDATORS             0

// 1 if use array for categories. Limits number of categories
// to 256, but improves GetPropertyCategory performance (used
// a lot in DoDrawItems).
// No significant code size difference.
// Recommended: Use 0, as is solves various issues.
#define wxPG_USE_CATEGORY_ARRAY         0
...
class WXDLLEXPORT wxOwnerDrawnComboBox;
# define wxCCustomComboBox wxOwnerDrawnComboBox
...
/** Indicates custom cell background colour is used (category only).
*/
//#define wxPG_PROP_CUSTCOLBG         0x0004
...
WX_PG_DECLARE_VALUE_TYPE_WITH_DECL(none,WXDLLIMPEXP_PG)
WX_PG_DECLARE_VALUE_TYPE_WITH_DECL(wxString,WXDLLIMPEXP_PG)
WX_PG_DECLARE_VALUE_TYPE_WITH_DECL(long,WXDLLIMPEXP_PG)
WX_PG_DECLARE_VALUE_TYPE_WITH_DECL(bool,WXDLLIMPEXP_PG)
WX_PG_DECLARE_VALUE_TYPE_WITH_DECL(double,WXDLLIMPEXP_PG)
WX_PG_DECLARE_VALUE_TYPE_WITH_DECL(void,WXDLLIMPEXP_PG)
WX_PG_DECLARE_VALUE_TYPE_WITH_DECL(wxArrayString,WXDLLIMPEXP_PG)
...
public:
    ...
    */
    ...
    /** Returns m_category. */
    inline unsigned int GetCatIndex() const { return (unsigned int)m_category; }

    /** Sets m_category. */
    inline void SetCatIndex(unsigned int ind) { m_category = (unsigned char)ind; }
#endif
    ...
    */
    ...
    inline void SetDataData ( void* clientData ) { m_clientData = clientData; }
#endif
...
#if wxPG_USE_CATEGORY_ARRAY
    // 0xFF if doesn't belong to a category and is not a category;
    // otherwise index to category table (if is category, then own index)
...
#if wxUSE_UNICODE || wxCHECK_GCC_VERSION(4,0)
...
#if wxPG_USE_CATEGORY_ARRAY
    unsigned char       m_ctIndex; // index in category table, 0xFF if not category
#endif
};
...
public:
    ...
    */
    ...
    {
        wxASSERT ( wxPGIdOk(id) );
        return wxPGIdToPtr(id)->SetClientData();
    }
    /** Sets client data (void*) of a property. */
    inline void SetPropertyClientData ( wxPGNameStr name, void* clientData )
    {
        return SetPropertyClientData(GetPropertyByName(name),clientData);
    }
#endif
...
public:
    ...
    */
    ...
    inline wxPGId GetPropertyCategory ( wxPGNameStr name )
    {
    ...
    */
    ...
    /** Inserts property to the list.
        ...
        While Append is always faster and more recommended way to
        add items, make note that when both data storages (categoric and
    ...
    */
    ...
    inline void SetPropertyUnspecified ( wxPGNameStr name )
    {
...
#if !wxPG_USE_CUSTOM_CONTROLS
    void OnComboItemPaint (wxOwnerDrawnComboBox* pCb,int item,wxDC& dc,
...
#if wxPG_USE_CATEGORY_ARRAY
    /** List of all category pointers (needed for fast category access). */
    wxArrayPtrVoid              m_catArray;
#endif
...
//private:
    ...
    */
    ...
    void SelectProperty ( wxPGProperty* p, bool focus = FALSE, bool forceswitch = FALSE );
...
#if wxPG_USE_CATEGORY_ARRAY
    wxPropertyCategoryClass* GetPropertyCategory ( wxPGProperty* p ) const;
#else
    ...
    void NavigateOut ( int dir ); // this will then call wxWindow::Navigate
