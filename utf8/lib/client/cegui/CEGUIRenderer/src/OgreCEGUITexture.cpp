...
void OgreCEGUITexture::loadFromMemory(const void* buffPtr, uint buffWidth, uint buffHeight)
{
    ...
    if (!d_ogre_texture.isNull() &&
        (d_ogre_texture->getFormat() == PF_A8R8G8B8 || d_ogre_texture->getFormat() == PF_B8G8R8A8) &&
        d_ogre_texture->getWidth() == buffWidth &&
        d_ogre_texture->getHeight() == buffHeight &&
        d_ogre_texture->getDepth() == 1 &&
        d_ogre_texture->getTextureType() == TEX_TYPE_2D)
    {
        HardwarePixelBufferSharedPtr hpb = d_ogre_texture->getBuffer();
        PixelBox box(buffWidth, buffHeight, 1, PF_A8R8G8B8, const_cast<void*>(buffPtr));
        hpb->blitFromMemory(box, box);
        StringUtil::StrStreamType str;
        str << "CEGUI Texture: reloaded texture " << d_ogre_texture->getName()
            << " size: " << buffWidth << "x" << buffHeight
            << " format: " << PixelUtil::getFormatName(d_ogre_texture->getFormat());
    ...
    uint32* swappedBuffer = (uint32*)(new uint8[bytesize]);
    memcpy(swappedBuffer, buffPtr, bytesize);

    for (int i=0; i < bytesize/4; i++)
        byteSwap(swappedBuffer[i]);

    DataStreamPtr odc(new MemoryDataStream(static_cast<void*>(swappedBuffer), bytesize, true));
#else
...
void OgreCEGUITexture::loadFromMemoryLA(const void* buffPtr, uint buffWidth, uint buffHeight)
{
    ...
    if (!d_ogre_texture.isNull() &&
        d_ogre_texture->getFormat() == TextureManager::getSingleton().getNativeFormat(TEX_TYPE_2D, PF_BYTE_LA, 0) &&
        d_ogre_texture->getWidth() == buffWidth &&
        d_ogre_texture->getHeight() == buffHeight &&
        d_ogre_texture->getDepth() == 1 &&
        d_ogre_texture->getTextureType() == TEX_TYPE_2D)
    {
        HardwarePixelBufferSharedPtr hpb = d_ogre_texture->getBuffer();
        PixelBox box(buffWidth, buffHeight, 1, PF_BYTE_LA, const_cast<void*>(buffPtr));
        hpb->blitFromMemory(box, box);
        StringUtil::StrStreamType str;
        str << "CEGUI Texture: reloaded texture " << d_ogre_texture->getName()
            << " size: " << buffWidth << "x" << buffHeight
...
int OgreCEGUITexture::getMemorySize(void) const
{
	return d_ogre_texture.isNull() ? 0 : d_ogre_texture->getBuffer()->getSizeInBytes();
}
