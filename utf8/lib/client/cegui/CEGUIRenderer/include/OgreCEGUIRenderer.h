...
public:
	...
	*/
	...
	virtual	unsigned long	addQuad(const Rect& dest_rect, float z, const Texture* tex, const Rect& texture_rect, const ColourRect& colours, QuadSplitMode quad_split_mode, bool need_quad_id=false);

	virtual void enterMultiTextureMode(const Rect& dest_rect, const Texture* tex, const Rect& texture_rect);
	virtual void exitMultiTextureMode(void);
	virtual void pauseMultiTextureMode(void);
	virtual void resumtMultiTextureMode(void);
	...
	*/
	...
	bool	isRenderDisable(void) const { return d_renderDisable; }
	void	setRenderDisable(bool bDisable) { d_renderDisable = bDisable; }
...
private:
	...
	************************************************************************/
    ...
    static const size_t    VERTEXBUFFER_WITH_MULTITEX_INITIAL_CAPACITY;		 //!< initial capacity of the allocated vertex buffer
	...
	*************************************************************************/
	...
	//binary predicates
	static bool	quadSortCriterion(const QuadInfo* q1, const QuadInfo* q2);
	...
	QuadInfo& getFreeQuadBufer(void);
	...
	*************************************************************************/
	...
	typedef std::list<QuadInfo> QuadBufer;
	QuadBufer d_quadBufer;
	QuadBufer::iterator d_freeQuad;
	...
	Ogre::LayerBlendModeEx		d_colourBlendMode1;	//!< Controls colour blending mode used.
	Ogre::LayerBlendModeEx		d_alphaBlendMode;	//!< Controls alpha blending mode used.
	Ogre::LayerBlendModeEx		d_alphaBlendMode1;	//!< Controls alpha blending mode used.
	Ogre::TextureUnitState::UVWAddressingMode d_uvwAddressMode; 
	bool						d_renderDisable;
	Ogre::VertexElementType		d_vertexElementType;
	...
	int							d_quadWithMultiTexCounts;
	const OgreCEGUITexture*		d_currentMultiTexture;
	bool						d_multiTextureMode;
	Rect						d_rectMultiTexDest;
	Rect						d_rectInMultiTexture;
	bool						d_multiTextureModePaused;
	Ogre::RenderOperation		d_render_op_withMultiTex;		//!< Ogre render operation we use to do our stuff.
	Ogre::HardwareVertexBufferSharedPtr	d_bufferWithMultiTex;	//!< vertex buffer to queue sprite rendering

	// fill vertex buff from quad list to vertex buf memory.
	void	_fillVertexBuf(void);
