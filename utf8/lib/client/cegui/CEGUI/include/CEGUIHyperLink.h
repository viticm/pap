...
namespace CEGUI
{
	...
	struct CEGUIEXPORT HyperLink
	{
		...
		void addNewRect(const Window* pOwnerWindow,const Rect newRect)
		{
			...
			d_rectBuf[d_length].d_left = d_rectBuf[d_length].d_left - 2;
			d_rectBuf[d_length].setWidth( d_rectBuf[d_length].getWidth() + 2 );
		...
		bool isInRange(const Window* pOwnerWindow,const Point& position, Rect* outRect = NULL)
		{
