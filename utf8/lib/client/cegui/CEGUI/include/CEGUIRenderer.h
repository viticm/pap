...
namespace CEGUI
{
	...
	*/
	...
	public:
		...
		*/
		virtual	unsigned long	addQuad(const Rect& dest_rect, float z, const Texture* tex, const Rect& texture_rect, const ColourRect& colours, QuadSplitMode quad_split_mode, bool need_quad_id=false) = 0;

		virtual void enterMultiTextureMode(const Rect& dest_rect, const Texture* tex, const Rect& texture_rect) = 0;
		virtual void exitMultiTextureMode(void) = 0;
		virtual void pauseMultiTextureMode(void) = 0;
		virtual void resumtMultiTextureMode(void) = 0;
		...
		*/
		...
		/*!
		\brief
		return the Z step value.

		\return
		float value that specifies the Z co-ordinate step value.
		*/
		float	getZElementStep(void) const		{return GuiZElementStep; }
