...
protected:
	...
	*/
	...
	\return
		Nothing.
	*/
	virtual void	createFontGlyphBorder(utf32 code);

	/*!
	\brief
		Function to do real work of constructor.  Used to save duplication in the various constructor overloads.
	*/
	virtual void	constructor_impl(const String& name, const String& fontname, const String& resourceGroup, uint size, bool antiAliase, bool encrypted);
	...
	\return
		Nothing.
	*/
	virtual void	drawGlyphToBuffer(utf32 code, uint16* buffer, uint buf_width, bool drawBorder);

	/*************************************************************************
		Implementation Data
	*************************************************************************/
	FontImplData*	d_impldat;		//!< Implementation data
	bool			d_antiAliase;	//!< Is anti-aliased
