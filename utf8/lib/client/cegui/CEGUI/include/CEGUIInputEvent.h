	...
	*/
	...
	public:
		...
		*/
		...
		{ 
			return strClassName==(const utf8*)"Window" ? true : EventArgs::isKindOf(strClassName);
		}
	...
	{
	public:
		HyperLinkEventArgs(const String& winName, const String32& hyperName) : windowName(winName), hyperLinkName(hyperName) {}
		/*!
		\brief
		Check event type name.
		*/
		virtual bool isKindOf(const String& strClassName) const 
		{ 
			return strClassName==(const utf8*)"HyperLink" ? true: EventArgs::isKindOf(strClassName);
		}

		String		windowName;
		String32	hyperLinkName;
	};
	...
	*/
	...
	public:
		...
		*/
		virtual bool isKindOf(const String& strClassName) const 
		{ 
			return strClassName==(const utf8*)"Mouse" ? true : WindowEventArgs::isKindOf(strClassName);
		}
	...
	*/
	...
	public:
		...
		*/
		virtual bool isKindOf(const String& strClassName) const 
		{ 
			return strClassName==(const utf8*)"MouseCursor" ? true : EventArgs::isKindOf(strClassName);
		}
	...
	*/
	...
	public:
		...
		*/
		virtual bool isKindOf(const String& strClassName) const 
		{ 
			return strClassName==(const utf8*)"Key" ? true : WindowEventArgs::isKindOf(strClassName);
		}
	...
	*/
	...
	public:
		...
		*/
		virtual bool isKindOf(const String& strClassName) const 
		{ 
			return strClassName==(const utf8*)"Activation" ? true : WindowEventArgs::isKindOf(strClassName);
		}
	...
	*/
	...
	public:
		...
		*/
		virtual bool isKindOf(const String& strClassName) const 
		{ 
			return strClassName==(const utf8*)"DragDrop" ? true : WindowEventArgs::isKindOf(strClassName);
		}
	...
	/*!
	\brief
	EventArgs based class used for ItemInfo notifications
	*/
	class CEGUIEXPORT ItemElementEventArgs : public WindowEventArgs
	{
	public:
		ItemElementEventArgs(Window* wnd) : WindowEventArgs(wnd),button(NoButton){msgID = -1;}
		/*!
		\brief
		Check event type name.
		*/
		virtual bool isKindOf(const String& strClassName) const
		{
			return strClassName==(const utf8*)"ItemElement" ? true : WindowEventArgs::isKindOf(strClassName);
		}

		int msgID;	//聊天历史信息id
		int	itemID;	//<! Transfer system's item element's id.
		MouseButton	button;			//!< one of the MouseButton enumerated values describing the mouse button causing the event (for button inputs only)
	};
