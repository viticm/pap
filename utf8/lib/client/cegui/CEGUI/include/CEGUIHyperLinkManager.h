...
namespace CEGUI
{
	...
	public:
		...
		*/
		...
		void destroyHyperLink(const String32 name);

		void addHyperLink(const Window* OwnerWindow, const String32 name, const Rect hyper_rect, utf32& ocolor);
		...
		void	clearMouseInLinkInfo(void)
		{
		...
		String32 getHyperLinkContex(const Window* pOwnerWindow, const Point& position);
	private:

		typedef std::map< String32, HyperLink > HyperLinkNameRegistry;
		//All HyperLink
		HyperLinkNameRegistry	d_HyperLinkNameMap;

		String32 d_lastMouseInLinkName;
		Window*		d_lastMouseInLinkWindow;
