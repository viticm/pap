...
namespace CEGUI
{
	...
	*/
	...
	public:
		...
		*/
		unsigned long	draw(const Rect& source_rect, const Rect& dest_rect, float z, const Rect& clip_rect,const ColourRect& colours, QuadSplitMode quad_split_mode, bool need_quad_id) const;
		...
		The id of render quad in render queue
		*/
		unsigned int	draw(const Rect& source_rect, const Rect& dest_rect, float z, const Rect& clip_rect, const colour& top_left_colour = 0xFFFFFFFF, const colour& top_right_colour = 0xFFFFFFFF,  const colour& bottom_left_colour = 0xFFFFFFFF, const colour& bottom_right_colour = 0xFFFFFFFF, QuadSplitMode quad_split_mode = TopLeftToBottomRight, bool need_quad_id=false) const
		{
			return draw(source_rect, dest_rect, z, clip_rect, ColourRect(top_left_colour, top_right_colour, bottom_left_colour, bottom_right_colour), quad_split_mode, need_quad_id);
		}
		...
		*/
		...
		/*!
		\brief
		Return the real texture file name.
		*/
		const String& getTextureFilename(void) const { return d_textureFilename; }

		/*!
		\brief
		Disable texture auto free function
		*/
		void disableAutoFree(void) { d_disableAutoFree = false; }

		/*!
		\breif
		Called by CEGUI Render when this imageset was rendered per tick	
		*/
		void increaseUsingSignal(void);

		/*!
		\brief
		Check is this texture unused lately and free it
		*/
		void autoFree(void);
...
#endif
		bool	isLoaded(void) const { return d_texture!=0; }
	...
	protected:
		...
		*************************************************************************/
		...
		// dynamic load fields
		bool	d_disableAutoFree;		//!< Disable auto free function
		int		d_inUsingSignal;		//!< Used times of this texture 
