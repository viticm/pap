...
class GUILayout_xmlHandler : public XMLHandler
{
	...
	typedef WindowManager::EventCallBack EventCallBack;
public:
	/*************************************************************************
	...
	*/
	GUILayout_xmlHandler(const String& name_prefix, PropertyCallback* callback = NULL, EventCallBack* eventcallback = NULL,void* userdata = NULL) : 
	  d_root(NULL),
	  d_namingPrefix(name_prefix),
	  d_propertyCallback(callback),
	  d_userData(userdata),
	  d_eventcallback(eventcallback)
	  {}
...
private:
	...
	*************************************************************************/
	...
	EventCallBack*		d_eventcallback;//!< Callback for every event loaded
