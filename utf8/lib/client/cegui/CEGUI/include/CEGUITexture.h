...
public:
	...
	*/
	...
	/*!
	\brief
		Loads (copies) an byte luminance-alpha format image in memory into the texture.  The texture is resized as required to hold the image.
	...
	*/
	...
	void setProxy(void* proxy) { d_proxy = proxy; }
	void* getProxy(void) const { return d_proxy; }
...
protected:
	...
	*/
	Texture(Renderer* owner) : d_owner(owner), d_proxy(0) {}
...
private:
	...
	void* d_proxy;			//!< Point to imageset or other.
};
