...
*************************************************************************/
ListboxItem* Listbox::findItemWithText(const String32& text, const ListboxItem* start_item)
{
	// if start_item is NULL begin search at begining, else start at item after start_item
	size_t index = (start_item == NULL) ? 0 : (getItemIndex(start_item) + 1);
...
void Listbox::onCharacter(KeyEventArgs& e)
{
	...
	while(1)
	{
		if ( str == d_listItems[d_curIndex]->getTagText())
		{
			...
			// 根据refIndex计算scrollbar的thumb移动的位置
			float pos = 0.0f;
