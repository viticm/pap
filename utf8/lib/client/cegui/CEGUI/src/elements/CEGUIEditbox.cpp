...
namespace CEGUI
{
	...
	*************************************************************************/
	...
	Window(type, name),
		...
		d_maskText(false),
		d_maskCodePoint('*'),
		d_maxTextLen(String32::max_size()),
		d_caratPos(0),
		d_selectionStart(0),
		d_selectionEnd(0),
		d_dragging(false),
	...
	*************************************************************************/
	...
	Rect Editbox::getCaratTextExtent() const
	{
		Rect rc = getPixelRect();
		if(d_text.empty() || 0 == d_caratPos) return rc;

		if(getFont())
		{
			float extentToCarat = getFont()->getTextExtent(d_text.substr(0, getCaratIndex()));
			rc.d_left += extentToCarat;
		}
	...
	*************************************************************************/
	...
	bool Editbox::is_FireFox_Utf32_Code(const utf32& code) const
	{
	...
	void Editbox::remove_FireFox_Utf32_Code(const String32& strIn, String32& strOut) const
	{
		strOut.erase();
	...
	expression.	
	*************************************************************************/
	bool Editbox::isStringValid(const String32& str) const
	{
		...
		// remove firfox only utf32 code
		String32 strNoFireFoxUtf32Code;
		remove_FireFox_Utf32_Code(str, strNoFireFoxUtf32Code);

		const char* utf8str = strNoFireFoxUtf32Code.c_str();
		int	match[3];
	...
	*************************************************************************/
	void     Editbox::addCharater( const utf32 &codepoint, bool isFromSoftKey )
	{
		if( !isVisible() )
			return;
		//*-*-*-* BigCharacter
