...
int TabControl::writeChildWindowsXML(OutStream& out_stream, const String& gap) const
{
    int childOutputCount = Window::writeChildWindowsXML(out_stream,gap);
    ...
    {
        getTabContentsAtIndex(i)->writeXMLToStream(out_stream, gap);
        ++childOutputCount;
    }
