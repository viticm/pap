...
*************************************************************************/
void MultiColumnList::addColumn(const String32& text, uint col_id, float width)
{
...
*************************************************************************/
void MultiColumnList::insertColumn(const String32& text, uint col_id, float width, uint position)
{
...
int MultiColumnList::writePropertiesXML(OutStream& out_stream, const String& gap) const
{
    ...
    int propCnt = Window::writePropertiesXML(out_stream, gap);
    ...
    // write out SortColumnID property, if any(!)
		try
		{
			...
			if (sortColumnID != 0)
			{
				  out_stream << gap <<"<Property Name=\"SortColumnID\" Value=\"" << PropertyHelper::uintToString(sortColumnID).c_str() << "\" />" << std::endl;
					++propCnt;
			}
		}
