...
namespace CEGUI
{
	...
	*************************************************************************/
	...
	const String Scrollbar::EventHasScrollToEnd( (utf8*)"HasScrollToEnd" );
	...
	*************************************************************************/
	void Scrollbar::initialise(void)
	{
		...
		d_increase->subscribeEvent(PushButton::EventMouseClick, Event::Subscriber(&CEGUI::Scrollbar::handleIncreaseClicked, this));

		// set up Decrease button
		d_decrease = createDecreaseButton(getName() + "__auto_decbtn__");
		addChildWindow(d_decrease);
		d_decrease->subscribeEvent(PushButton::EventMouseClick, Event::Subscriber(&CEGUI::Scrollbar::handleDecreaseClicked, this));
	...
	*************************************************************************/
	void Scrollbar::setScrollPosition(float position)
	{
		...
		if (d_position != old_pos)
		{
			...
			if(d_position == max_pos)
			{
				fireEvent(EventHasScrollToEnd, args, EventNamespace);
			}
		}
	...
	void Scrollbar::changeStateOfButtons(void)
	{
		// do nothing
	}
