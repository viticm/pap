...
namespace CEGUI
{
	...
	namespace FalagardSimpleTextProperties
	{
		...
		{
			char szRet[256] = {0};
			Size sz = static_cast<const FalagardSimpleText*>(receiver)->getTextScale();
			sprintf( szRet, "x:%d y:%d", (int)sz.d_width, (int)sz.d_height);
			return szRet;
		}
		...
		{
			return PropertyHelper::colourToString(static_cast<const FalagardSimpleText*>(receiver)->getTextColor());
		}
