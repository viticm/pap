...
//更改代码，添加标记 (_WX7@)
#define WidthPerPic 256
#define HeightPerPic 256
...
namespace CEGUI
{
	...
	{
		int nSceneID = PropertyHelper::stringToInt( (((const WindowEventArgs&) e).window)->getText().c_str() );
	...
	void FalagardWorldMap::createBackground()
	{
		...
		pos.d_y = 0.01;
		m_pBackground->setSize( Relative, size );
		m_pBackground->setPosition( Relative, pos );
		/************************************************************************/
	...
	void FalagardWorldMap::resetWorldMap()
	{
		...
		for( it = m_aCityList.begin(); it != m_aCityList.end(); it ++ )
		{
			...
			if( itType != m_CityDefineList.end() )
			{
				...
				tranpoint(data->pCityWindow, data->cityPos, city.pNormalImage );
				...
				tranpoint( data->pCityNameWindow, data->namePos, data->pNameNormalImage );
	...
	void FalagardWorldMap::setCityTooltipText(int nCityID, const String32 &szCityTip)
	{
		CITY_DATA* data = getCity( nCityID );
		data->pTooltip = szCityTip;
		data->pCityWindow->setTooltipText( data->pTooltip );
	}

	void FalagardWorldMap::setCityNameTooltipText( int nCityID, const String32& szCityNameTip )
	{
		CITY_DATA* data = getCity( nCityID );
		data->pTooltip = szCityNameTip;
		data->pCityNameWindow->setTooltipText( data->pTooltip);
	}

	void FalagardWorldMap::tranpoint( Window* wnd, const Point point, Image* image )
	{
		int width = m_BackGroundSize.d_width * WidthPerPic;
		int height = m_BackGroundSize.d_height * HeightPerPic;

		Size AbsSize = image->getSourceTextureArea().getSize();
		Size RelSize;
		RelSize.d_width = ((float)AbsSize.d_width/width)*getAbsoluteWidth();
		RelSize.d_height = ((float)AbsSize.d_height/height)*getAbsoluteWidth();

		Point Relposition;
		Relposition.d_x = ((float)point.d_x/width)*getAbsoluteWidth();
		Relposition.d_y = ((float)point.d_y/height)*getAbsoluteWidth();

		wnd->setHookPosition( Relposition );
		wnd->setSize( Absolute, RelSize );
