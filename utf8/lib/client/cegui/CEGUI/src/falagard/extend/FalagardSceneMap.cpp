...
namespace CEGUI
{
	...
	FalagardSceneMapProperties::PlayerImage			FalagardSceneMap::d_PlayerImageProperty;		// 玩家用得图标
	...
	FalagardSceneMapProperties::ImageNameRuler			FalagardSceneMap::d_ImageNameRulerProperty;		// 图片命名规则
	...
	void FalagardSceneMap::updateSelf(float elapsed)
	{
		if( d_fresh && isVisible() )
		{
			...
			String32 strTooltip[ 5 ]; //5个人得tooltip
	...
	const Image* FalagardSceneMap::getImageByName( const String& str,const String& strPostFix,Rect rect ) // 根据名字得到image
	{
