...
// 绘制裁剪区域内的图片		20100607 BLL
void RenderableElement::draw( RenderCache& renderCache, const Rect& clip_rect ) const
{
	draw_impl( renderCache, clip_rect );

	// also draw next element in the chain
	if (d_next)
	{
		d_next->draw( renderCache, clip_rect );
	}
}
