...
bool System::injectMouseMove(float delta_x, float delta_y)
{
	...
	if (dest_window != NULL)
	{
		//Hack by jinchao: MouseHollow (Ensure the window with mouse process MouseLeave Event!).
...
bool System::injectMouseButtonDown(MouseButton button)
{
	...
	if(dest_window && dest_window->testClassName((CEGUI::utf8*)"Listbox"))
		d_activeWindow = dest_window;
	else
		d_activeWindow = NULL;
...
bool System::injectMouseButtonUp(MouseButton button)
{
	...
	//toooooooooooooooo bad. wait a better solution. yangjun [4/8/2006]
...
bool System::injectChar(utf32 code_point)
{
	...
	if (d_activeSheet != NULL)
	{
		...
		if( Ime::HasInputFocus() ) 
		{
			dest = Ime::GetActiveIMEEditBox();
		}
		else
		{
			if( d_activeWindow == NULL )
