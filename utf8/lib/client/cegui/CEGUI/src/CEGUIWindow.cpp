...
WindowProperties::AchorWindow		Window::d_achorWindowProperty;
WindowProperties::AchorMode			Window::d_achorModeProperty;
WindowProperties::AchorOffset		Window::d_AchorOffsetProperty;
WindowProperties::ShowTooltip		Window::d_ShowTooltipProperty;
/*************************************************************************
	...
	d_windowFactory(WindowFactoryManager::getSingleton().getFactory(d_type))
{
	...
	d_mouseMoveHollow = false;		// 鼠标移动透过，点击也透过
	...
	d_achorWindow = NULL;
	d_parentAchorWindow = NULL;
	d_achorMode = AM_NONE;
	d_achorOffset = Point(0.0f,0.0f);
...
bool	Window::isCanVisible(void) const
{
	Rect rectScreen = System::getSingleton().getRenderer()->getRect();
	Rect rectSelf = getPixelRect();
	Rect inter = rectScreen.getIntersection(rectSelf);
...
*************************************************************************/
void Window::setTextOriginal(const String32& text)
{
	d_textOriginal = text;
}

void Window::setText(const String32& text)
{
	d_text = text;
    WindowEventArgs args(this);
	onTextChanged(args);
...
void Window::setAchorWindow( const String& name )
{
	try
	{
		Window* achorWindow = WindowManager::getSingleton().getWindow(name);
		achorWindow->d_achorWindow = this;
		setAchorParentWindow(achorWindow);
	}
	catch (...)
	{
		// no window ignore
...
const String Window::getAchorWindow(void) const
{
	if(d_achorWindow)
	{
		return d_achorWindow->getName();
	}
	return "";
}

const	String	Window::getAchorOffset(void) const
{
	char szMsg[128] = {0};
	sprintf(szMsg, "x:%0f,y:%0f", d_achorOffset.d_x, d_achorOffset.d_y);
	return szMsg;
}
void			Window::setAchorOffset(Point pt)
{
	d_achorOffset = pt;
}

void Window::onAchorWindowDestory(void)
{
	d_achorWindow = NULL;
}

void Window::executeAchor(void)
{
	Point pos;
	if( d_achorWindow )
	{
		switch(d_achorWindow->getAchorMode())
		{
		case AM_LEFT:
			{
				pos.d_y = getPosition(CEGUI::Absolute).d_y;
				pos.d_x = getPosition(CEGUI::Absolute).d_x - d_achorWindow->getAbsoluteWidth();
			}
			break;
		case AM_RIGHT:
			{
				pos.d_y = getPosition(CEGUI::Absolute).d_y;
				pos.d_x = getPosition(CEGUI::Absolute).d_x + getAbsoluteWidth();
			}
			break;
		case AM_TOP:
			{
				pos.d_y = getPosition(CEGUI::Absolute).d_y - d_achorWindow->getAbsoluteHeight();
				pos.d_x = getPosition(CEGUI::Absolute).d_x;
			}
			break;
		case AM_BOTTOM:
			{
				pos.d_y = getPosition(CEGUI::Absolute).d_y + getAbsoluteHeight();
				pos.d_x = getPosition(CEGUI::Absolute).d_x;
			}
			break;
		case AM_NONE:
		...
		pos += d_achorWindow->d_achorOffset;
		d_achorWindow->setPosition(CEGUI::Absolute, pos);
	}
}
...
void Window::addStandardProperties(void)
{
	CEGUI_START_ADD_STATICPROPERTY( Window );
		...
		CEGUI_ADD_STATICPROPERTY( &d_achorWindowProperty);
		CEGUI_ADD_STATICPROPERTY( &d_achorModeProperty);
		CEGUI_ADD_STATICPROPERTY( &d_AchorOffsetProperty);
		CEGUI_ADD_STATICPROPERTY( &d_ShowTooltipProperty);
...
void Window::update(float elapsed)
{
	...
	//不知道为什么,有的时候child_count会大于getChildCount()的值,难道是update函数,删除了子窗口??
...
void Window::destroy(void)
{
	...
	if( d_parentAchorWindow)
	{
		d_parentAchorWindow->onAchorWindowDestory();
		d_parentAchorWindow = NULL;
	}
...
void Window::setTooltipText(const String32& tip)
{
    d_tooltipText = tip;
...
const String32& Window::getTooltipText(void) const
{
	if( !d_showTooltips ) return String32();
...
// ==============================================
// setAreaAndTexCoord
// fXPercent和fYPercent：x和y方向上减小百分比
// iDir_X和iDir_Y：图片在x和y方向上的缩减方向
// iDir_X：从左向右和从右向左
// iDir_Y：从上向下和从下向上
...
void Window::writeXMLToStream(OutStream& out_stream, const String& gap) const
{
    // output opening Window tag
    out_stream << gap << "<Window Type=\"" << getType() << "\" ";
    ...
    writePropertiesXML(out_stream,gap);
    // write out attached child windows.
    writeChildWindowsXML(out_stream,gap);
    // now ouput closing Window tag
    out_stream << "</Window>" << std::endl;
}

int Window::writePropertiesXML(OutStream& out_stream, const String& gap) const
{
	//return 0;
#if 1
    int propertiesWritten = 0;
	PropertySet::PropertyIterator iter =  d_windowFactory->getIterator();
    ...
    while(!iter.isAtEnd())
    {
        ...
        {
            iter.getCurrentValue()->writeXMLToStream(this, out_stream,	gap);
            ++propertiesWritten;
        }
			}
...
int Window::writeChildWindowsXML(OutStream& out_stream, const String& gap) const
{
    ...
    for (uint i = 0; i < getChildCount(); ++i)
    {
        ...
        {
            child->writeXMLToStream(out_stream, gap + "\t");
            ++windowsWritten;
        }
    }
...
void Window::onShown(WindowEventArgs& e)
{
	...
	if(d_parentAchorWindow)
	{
		d_parentAchorWindow->executeAchor();
	}
...
void  Window::setName(const utf8* szName)
{
	WindowManager::getSingleton().renameWindow(this, szName);
}
