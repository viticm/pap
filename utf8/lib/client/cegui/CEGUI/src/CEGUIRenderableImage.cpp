...
void RenderableImage::draw_impl(RenderCache& renderCache) const
{
    ...
    for (uint row = 0; row < vertTiles; ++row)
    {
		...
		if (d_horzFormat == HorzTiled && targetRect.d_right > d_area.d_right)
		{
			targetRect.d_right = d_area.d_right;
		}
...
void RenderableImage::draw_impl( RenderCache& renderCache, const Rect& clip_rect ) const
{
	// do not do anything if image is not set.
	if (!d_image)
		return;
	...
	// calculate final clipping rect which is intersection of RenderableImage area and supplied clipping area
	Rect final_clipper( d_area.d_left, d_area.d_top, d_area.d_right, d_area.d_bottom );
	final_clipper.setSize( d_area.getSize() );
	final_clipper = clip_rect.getIntersection(final_clipper);
	...
	for( uint row = 0; row < vertTiles; ++row )
	{
		...
		for ( uint col = 0; col < horzTiles; ++col )
		{
			...
			// cache the image
			renderCache.cacheImage( *d_image, targetRect, 0, final_colours, &final_clipper, true );
