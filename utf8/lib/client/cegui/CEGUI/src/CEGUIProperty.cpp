...
namespace CEGUI
{
	...
	*************************************************************************/
    ...
    void Property::writeXMLToStream(const PropertyReceiver* receiver, OutStream& out_stream, const String& gap) const
    {
        if (d_writeXML)
        {
            out_stream << gap << "<Property Name=\"" << d_name <<"\" Value=\"" << get(receiver).c_str() << "\" />"  << std::endl;
        }
    }
