...
namespace CEGUI
{
	...
	void RenderCache::render(const Point& basePos, float baseZ, const Rect& clipper) const
	{
		...
		for(ImageryList::const_iterator image = d_cachedImages.begin(); image != d_cachedImages.end(); ++image)
		{
			...
			if( !imageInfo.source_image ) continue;

			if (imageInfo.usingCustomClipper)
			{
				custClipper = imageInfo.customClipper;
				custClipper.offset(basePos);
				custClipper = imageInfo.clipToDisplay ? displayArea.getIntersection(custClipper) : clipper.getIntersection(custClipper);
				finalClipper = &custClipper;
			}
			else
			{
				finalClipper = imageInfo.clipToDisplay ? &displayArea : &clipper;
			}

			finalRect = imageInfo.target_area;
			finalRect.offset(basePos);
			imageInfo.source_image->draw(finalRect, baseZ + imageInfo.z_offset, *finalClipper, imageInfo.colours);
		}

		// advance text Z co.
		baseZ -= System::getSingleton().getRenderer()->getZElementStep()*0.2f;
		...
		for(TextList::const_iterator text = d_cachedTexts.begin(); text != d_cachedTexts.end(); ++text)
		{
			...
			finalRect.offset(basePos);
			(*text).source_font->drawText((const Window*)(*text).OwnerWindow, (const String32&)(*text).text, 
	...
	void RenderCache::cacheText( const Window* OwnerWindow, const String32& text, const FontBase* font, TextFormatting format, const Rect& destArea, float zOffset, const ColourRect& cols, const Rect* clipper, bool clipToDisplay, float fScaleX, float fScaleY )
	{
		TextInfo txtinf;
