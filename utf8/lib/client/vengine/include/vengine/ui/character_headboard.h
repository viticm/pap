...
class VENGINE_API CharacterHeadBoard {
 ...
 public:
   ...
   virtual void settitle(const char* name) = 0; //设置称号
   virtual void setname(const char* name) = 0; //设置名称
   virtual void set_HPProcess(uint32_t now, uint32_t max) = 0; //设置血量
   virtual void set_HPProcess(float percent) = 0; //血量百分比
   virtual void set_countryflag(uint8_t flag) = 0; //国家标记--以后变为阵营
   virtual void set_leaderflag(uint8_t flag) = 0; //队长标记
   virtual void set_pkflag(uint8_t flag) = 0; //设置PK|任务标记
