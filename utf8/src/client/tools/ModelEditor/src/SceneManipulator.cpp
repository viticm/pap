...
void
SceneManipulator::newScene(const String& texName)
{
    _preprocessScene();

	createGrid();
	createTerrain(texName);
    _postprocessScene();
...
void SceneManipulator::createTerrain(const String& tex)
{
	_createTerrain(16,16,32,tex,0,100,100);
}
void SceneManipulator::setTerrainVisible(bool visible)
{
	mTerrainVisible = visible;
...
void SceneManipulator::setGridVisible(bool visible)
{
	mGridVisible = visible;

	Ogre::ManualObject* gridObject = getSceneManager()->getManualObject("gridObject");
	if (gridObject)
	{
		gridObject->setVisible(mGridVisible);
	}
}
void SceneManipulator::createGrid()
{
	...
	plusXAxisMaterial->setDepthCheckEnabled(false);
	plusXAxisMaterial->setDepthWriteEnabled(false);
	...
	negativeXAxisMaterial->setDepthCheckEnabled(false);
	negativeXAxisMaterial->setDepthWriteEnabled(false);
	...
	plusYAxisMaterial->setDepthCheckEnabled(false);
	plusYAxisMaterial->setDepthWriteEnabled(false);
	...
	negativeYAxisMaterial->setDepthCheckEnabled(false);
	negativeYAxisMaterial->setDepthWriteEnabled(false);
	...
	plusZAxisMaterial->setDepthCheckEnabled(false);
	plusZAxisMaterial->setDepthWriteEnabled(false);
	...
	negativeZAxisMaterial->setDepthCheckEnabled(false);
	negativeZAxisMaterial->setDepthWriteEnabled(false);
