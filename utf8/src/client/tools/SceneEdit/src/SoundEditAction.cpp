...
namespace Fairy
{
    ...
    void SoundEditAction::_createIndicatorInstance(void)
    {
        ...
        Ogre::MeshPtr sphereMesh = Fairy::createCommonSphere("SoundRadiusEntity", radius, rings, segments);
        Ogre::MaterialPtr material = Fairy::createPureColourMaterial(
    ...
    void SoundEditAction::_destroyIndicatorInstance(void)
    {
        if (mFirstInit)
        {
            ...
            mSoundMovingEntity->getParentSceneNode()->getCreator()->destroyEntity(mSoundMovingEntity);
			mSoundMovingEntity = NULL;

            mSoundRadiusEntity->getParentSceneNode()->getCreator()->destroyEntity(mSoundRadiusEntity);
			mSoundRadiusEntity = NULL;
        }
    }
    ...
    void SoundEditAction::_clearAllSoundEntities(void)
    {
        for (size_t i=0; i<mSoundEntities.size(); ++i)
        {
            ...
            mSoundNode->getCreator()->destroyEntity(entity);
        }
