...
namespace Fairy
{
   ...
   void AxisRenderable::updateCamera(Ogre::Camera* cam)
   {
	   mCamera = cam;
	   if (mCamera)
	   {
   ...
   AxisRenderable::renderAxisText()
   {
       for(int i=0;i<3;i++)
       {
		   ...
		   {
			    Ogre::Matrix4 viewProjMatrix = mCamera->getProjectionMatrix() * mCamera->getViewMatrix();
				Ogre::Vector3 topLeft = viewProjMatrix*(textPos-Ogre::Vector3(32,0,32));
				Ogre::Vector3 bottomRight = viewProjMatrix*(textPos+Ogre::Vector3(32,0,32));
				left = topLeft.x;
				top = topLeft.y;
				right = bottomRight.x;
