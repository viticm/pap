...
namespace Fairy
{
    ...
    public:
		...
		// 选择一个点
		void _fireRegionSelectPoint(unsigned long ulRegionId, unsigned long ulPointId, SceneListener* exclude = 0);
		...
		// 调用所有移动怪物的事件响应函数
		void _fireMoveMonster( unsigned long ulInstanceId, Ogre::Vector3& postion, SceneListener* exclude = 0);
		...
		void _fireSelectableChanged(const Ogre::String& objType, bool bSelectable = true , SceneListener* exclude = NULL);
		void _fireSelectableChanged(const ObjectPtr& object, bool bSelectable = true , SceneListener* exclude = NULL);

		void _fireShowHintChanged(bool bShow = true, SceneListener* exclude = NULL);
    };
}
