...
namespace Fairy 
{
	...
	public:
		...
		Ogre::SceneNode*	m_pLocalSceneNode;		// 用来显示本地区域编辑的总的节点.
	...
	public:
		...
		// LocalArea显示矩形列表
		SHOW_RECT_MAP			  m_LocalRectShowMap;
		...
		// 不带redo undo 操作的删除
		void DeleteRectNoOperator(int iId, bool bLocal = false);

		// 添加一个矩形.
		int AddRect(Ogre::Vector3 vectorUnDoPos, Ogre::Vector3 vectorReDoPos,Ogre::StringVector nameVector, Ogre::StringVector valueVector, int iId = -1, bool bLocal = false);
    ...
    protected:
		...
		void _readAreaFromSingleFile(const std::string& fileName, bool bLocal = false);
		void _writeAreaFromSingleFile(const std::string& fileName, SHOW_RECT_MAP& map);
