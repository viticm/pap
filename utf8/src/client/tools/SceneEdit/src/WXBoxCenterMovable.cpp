...
namespace Fairy {
	...
	BoxCenterManager::init()
	{
		...
		for(iter = mObjBoxCenterMap.begin();iter != mObjBoxCenterMap.end();iter++)
		{
			...
			{
				Ogre::SceneNode* pParentNode = mv->getParentSceneNode();
				pParentNode->getCreator()->destroySceneNode(pParentNode);
				pParentNode = NULL;
			}
    ...
    { 
		...
		if (m_pCamera->getProjectionType() == Ogre::PT_ORTHOGRAPHIC)
		{
			Ogre::Matrix4 viewProjMatrix = m_pCamera->getProjectionMatrix() * m_pCamera->getViewMatrix();
			Ogre::Vector3 topLeft = viewProjMatrix*(mCenterPosWC-Ogre::Vector3(PointPixelSize,0,PointPixelSize));
			Ogre::Vector3 bottomRight = viewProjMatrix*(mCenterPosWC+Ogre::Vector3(PointPixelSize,0,PointPixelSize));
			left = topLeft.x;
			top = topLeft.y;
			right = bottomRight.x;
	...
	//david-<<
	void BoxCenterMovable::visitRenderables(Ogre::Renderable::Visitor* visitor, 
    ...
    BoxCenterMovable::getBoundingBoxCenter()
    {
		...
		else if (!object.lock()->hasProperty("bounding box") && object.lock()->hasProperty("position"))
		{
			Ogre::Vector3 pos = VariantCast<Ogre::Vector3>(object.lock()->getProperty("position"));
			mCenterPosWC = pos;
		}
    }
