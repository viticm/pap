...
namespace Fairy 
{
	...
	void CEventAreaEditOperator::SetAction(CEventAreaEditAction* pAction)
	{
		...
		// 设置区域ID
	void CEventAreaEditOperator::SetInstanceId(unsigned long ulInstanceId, bool bLocal)
	{
	...
	void CEventAreaEditDelOperator::undo(void)
	{
		if(m_pAction)
		{
			m_ulInstanceId = m_pAction->AddRect(m_vectorUnDoPos, m_vectorReDoPos, m_paramName, m_paramValue, m_ulInstanceId, m_bLocalArea);
	...
	void CEventAreaEditDelOperator::redo(void)
	{
		if(m_pAction)
		{
			m_pAction->DeleteRectNoOperator(m_ulInstanceId, m_bLocalArea);
		}
	}
