...
namespace Fairy
{
    ...
    void DatuPointEditAction::_createIndicatorInstance(void)
    {
        ...
        Ogre::MeshPtr sphereMesh = Fairy::createCommonSphere("DatuPointRadiusEntity", radius, rings, segments);
    ...
    void DatuPointEditAction::_destroyIndicatorInstance(void)
    {
        if (mFirstInit)
        {
			...
			//david->>
            mSoundNode->getCreator()->destroyEntity(mSoundMovingEntity);
			mSoundMovingEntity = NULL;

            mSoundNode->getCreator()->destroyEntity(mDatuPointRadiusEntity);
			mDatuPointRadiusEntity  = NULL;
        }
    }
