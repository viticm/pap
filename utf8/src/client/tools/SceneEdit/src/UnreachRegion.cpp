...
namespace Fairy 
{
	...
	unsigned long CPointInfo::CreateEntity()
	{
        ...
        if((m_pSceneManipulator)&&(m_pSceneManipulator->getSceneManager()))
		{
			...
			pObjectPtr = Fairy::ObjectFactoryManager::getSingleton().createInstance("REGION");
			Ogre::String groupId = Ogre::StringConverter::toString(m_ulParentId);
			pObjectPtr->setData(m_ulPointId);
			pObjectPtr->setName(groupId);
			mProxy= new Fairy::ObjectProxy();
			mProxy->setObject(pObjectPtr);
			Fairy::ObjectPtr object = mProxy->getObject();
			//david-<<
			m_pEntity->setUserAny(Ogre::Any(mProxy));
			//david->>
	...
	void CPointInfo::SetCurMaterial()
	{
		SetMaterial("Region/PointCur");
	}
	...
	CUnreachRegion::CUnreachRegion(void)
		...
		, m_pLineShape1(NULL)
		, m_bFlyEnable(false)
		, m_vBasePoint(Ogre::Vector3::ZERO)
	...
	void CUnreachRegion::SetAllPointMaterial()
	{
		if(m_pointMap.size() > 0)
		{
			POINT_MAP::const_iterator it = m_pointMap.begin();
			for(;it != m_pointMap.end();it++)
			{
				CPointInfo* pPointInfo = (*it).second;
				if(pPointInfo)
				{
					pPointInfo->SetMaterial("Region/RegionSel");
				}
		...
		if(m_pointMap1.size() > 0)
		{
			POINT_MAP::const_iterator it = m_pointMap1.begin();
			for(;it != m_pointMap1.end();it++)
			{
				CPointInfo* pPointInfo = (*it).second;
				if(pPointInfo)
				{
	...
	// 设置区域编辑状态材质.
	void CUnreachRegion::SetRegionEditMaterial()
	{
		//SetRegionMaterial("Region/RegionEdit");
		SetAllPointMaterial();
		SetLineNorColor();
		m_AreaShape.setMaterial("Region/AreaRegionEdit");
		HideAreaNode(true);

	}
	void CUnreachRegion::SetRegionEditMaterial1()
	{
		//SetRegionMaterial("Region/RegionEdit");
		SetAllPointMaterial();
		SetLineNorColor();
		m_AreaShape1.setMaterial("Region/AreaRegionEdit");
		HideAreaNode1(true);
...
bool CUnreachRegion::IsMouseSel(float x, float y)
{
	...
	if(pCamera)
	{
		...
		if (pCamera->getProjectionType() == Ogre::PT_ORTHOGRAPHIC)
		{
			...
			v.x =  ( ( ( 2.0f * x + 2) / iViewWidth ) - 1);
			v.y =  -( ( ( 2.0f * y + 2) / iViewHeight) - 1);
			v.z =  -1.0f;

			dir = Ogre::Vector3::NEGATIVE_UNIT_Y;
			orig = viewProjMatrix.inverse()*v;
		}
		else
		{
...
//基本参考点
Ogre::Vector3& CUnreachRegion::getBasePoint()
{
	return m_vBasePoint;
}

bool CUnreachRegion::hasBasePoint()
{
	return m_bHasBasePoint;
}
