...
namespace Fairy 
{
	...
	class CRegionEditAction : public Action, protected SceneListener
	{
	...
	public:
		...
		unsigned long m_ulCurSelectPointId;			// 当前正在选择的点的id
	...
	public:
		...
		//编辑一个选择的区域
		void EditSelRegion(unsigned long ulRegionId);

		//完成创建一个区域

		void FinishNewRegion(unsigned long ulRegionId);
		...
		// 设置一个点为当前点.
		void SetPoint(unsigned long ulPointId);
		...
		// 变换编辑区域
		void ChangeCurEditRegion(unsigned long ulRegionId,bool bFireUI=false,bool bIsChangeCamera = true);
