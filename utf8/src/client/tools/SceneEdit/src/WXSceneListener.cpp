...
namespace Fairy
{
	...
	// 选择编辑区域中的点
	void SceneListener::onSelectPoint(unsigned long ulRegionId,unsigned long ulPointId)
	{
	...
	// 移动一个怪物
	void SceneListener::onMoveMonster( unsigned long ulMonsterId, Ogre::Vector3& pos)
	{
	...
	void SceneListener::onSelectableChanged(const Ogre::String& objType, bool bSelectable)
	{

	}

	void SceneListener::onSelectableChanged(const ObjectPtr& object, bool bSelectable)
	{

	}

	void SceneListener::onShowHintChanged( bool bShow )
	{
	...
	void SceneListenerManager::_fireRegionSelectPoint(unsigned long ulRegionId, unsigned long ulPointId, SceneListener* exclude)
	{
		for (Listeners::const_iterator it = mListeners.begin(); it != mListeners.end(); ++it)
		{
			SceneListener* listener = *it;
			if (listener != exclude)
			{
				listener->onSelectPoint(ulRegionId, ulPointId);
			}
		}
	...
	// 调用所有移动怪物的事件响应函数
	void SceneListenerManager::_fireMoveMonster( unsigned long ulInstanceId,Ogre::Vector3& postion,SceneListener* exclude)
	{
		for (Listeners::const_iterator it = mListeners.begin(); it != mListeners.end(); ++it)
		{
			SceneListener* listener = *it;
			if (listener != exclude)
			{
				listener->onMoveMonster(ulInstanceId,postion);
			}
		}
	...
	void SceneListenerManager::_fireSelectableChanged(const Ogre::String& objType, bool bSelectable, SceneListener* exclude)
	{
		for (Listeners::const_iterator it = mListeners.begin(); it != mListeners.end(); ++it)
		{
			SceneListener* listener = *it;
			if (listener != exclude)
				listener->onSelectableChanged(objType, bSelectable);
		}
	}

	void SceneListenerManager::_fireSelectableChanged(const ObjectPtr& object, bool bSelectable , SceneListener* exclude)
	{
		for (Listeners::const_iterator it = mListeners.begin(); it != mListeners.end(); ++it)
		{
			SceneListener* listener = *it;
			if (listener != exclude)
				listener->onSelectableChanged(object, bSelectable);
		}
	}

	void SceneListenerManager::_fireShowHintChanged(bool bShow, SceneListener* exclude)
	{
		for (Listeners::const_iterator it = mListeners.begin(); it != mListeners.end(); ++it)
		{
			SceneListener* listener = *it;
			if (listener != exclude)
				listener->onShowHintChanged(bShow);
		}
	}
