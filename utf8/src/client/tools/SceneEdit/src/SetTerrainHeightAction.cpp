...
namespace Fairy {
    ...
    SetTerrainHeightAction::SetTerrainHeightAction(SceneManipulator* sceneManipulator)
        : HeightAction(sceneManipulator), mTerrainHeight(1000), mTerrainHeightIncrement(0),mMouseCurrentPos(0,0)
    {
    ...
    void SetTerrainHeightAction::setParameter(const String& name, const String& value)
    {
		...
		else if (name == "%CopyCurrentTerrainHeight") // 拷贝鼠标当前位置地形高度
		{
			copyCurrentMousePosTerrainHeight();
		}
        else
        {
    ...
    void SetTerrainHeightAction::copyCurrentMousePosTerrainHeight()
	{
		Ogre::Vector3 position;
		bool intersected = getSceneManipulator()->getTerrainIntersects(mMouseCurrentPos, position);
		if (intersected)
		{
			mTerrainHeight = position.y;
		}
	}
}
