...
namespace Fairy 
{
	...
	// 记录当前操作前一次点的位置.
	void RecordPreDoMonsterPos();

	// 当前操作之前的模型的位置.
	Ogre::Vector3 m_preDoMonsterPos;

	// 当前操作之前的模型的位置.
	Ogre::Vector3 m_curMonsterPos;

	//移动当前模型
	void ChangeCurEditMonsterPos(const Point& pt);


	public:

	bool m_bMonsterMoving;
	...
	//移动一个怪物
	void MoveMonster(Ogre::Vector3& pos, unsigned long dwInstanceId);
