...
namespace Fairy {
	...
	void AddParticleSystemObjectPlugin::onSelectableChanged(const Ogre::String& objType, bool bSelectable)
	{
		if ( objType == "ParticleSystem")
		{
			for (Indicators::iterator i = mIndicators.begin(); i != mIndicators.end(); ++i )
			{
				if ( i->second )
	...
	void AddParticleSystemObjectPlugin::onSelectableChanged(const ObjectPtr& object, bool bSelectable)
	{
