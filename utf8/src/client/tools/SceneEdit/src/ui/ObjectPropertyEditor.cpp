...
ObjectPropertyEditor::OnPropertyChanged(wxPropertyGridEvent& e)
{
    ...
    Fairy::String propertyName = AS_STRING(e.GetPropertyName());
    Fairy::String propertyValue = AS_STRING(e.GetPropertyValueAsString());

    Fairy::Variant oldValue = object->getProperty(propertyName);
    ...
    Fairy::Variant newValue = object->getProperty(propertyName);

	// 实现属性orientation和orientationXYZ的联动
	if (propertyName == "orientation")
	{
		wxPGId id = mPropertiesViewer->GetPropertyByName("orientationXYZ");
		wxPGIdToPtr(id)->SetValueFromString(AS_STRING(object->getPropertyAsString("orientationXYZ")), wxPG_FULL_VALUE);
	}
	else if (propertyName == "orientationXYZ")
	{
		wxPGId id = mPropertiesViewer->GetPropertyByName("orientation");
		wxPGIdToPtr(id)->SetValueFromString(AS_STRING(object->getPropertyAsString("orientation")), wxPG_FULL_VALUE);
	}

    Fairy::ModifyObjectPropertyOperator* op = new Fairy::ModifyObjectPropertyOperator(mSceneManipulator);
