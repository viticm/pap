    ...
    , mDatuPointEditDialog(NULL)
{
	...
	notebook->AddPage(mSmartColourDialog, wxT("顶点刷颜色"));
...
FairyEditorFrame::InitMenuBar(void)
{
	...
	file_menu->Append(feID_IMPORT_HEIGHT_MAP,_("导入高度图"),
		_("Import height map."));
	file_menu->Append(feID_EXPORT_HEIGHT_MAP,_("导出高度图"),
		_("Export heitht map."));
	...
	tools_menu->AppendSeparator();
	tools_menu->Append(feID_CONVERT_CBRUSH_INFO, _("读取cbrush文件数据"), _("为兼容之前的版本，读取cbrush文件数据用于保存"));
	...
	creator_menu->Append(feID_CREATE_OBJECT_ARRAY,
		/*_("Create Light Object"),*/_("创建物体队列"),
		/*	_("Create a light object"));*/_("创建物体队列"));
...
FairyEditorFrame::OnCopy(wxCommandEvent& event)
{
    ...
    if (currentAction)
    {
		...
		// Ctrl + C // 设置地形高度状态下 // 拷贝当前鼠标位置处地形高度 
...
{
	int answer = wxMessageBox(_("载入刷子文件会覆盖当前的顶点色数据，确定要载入吗？"), _("确认"),
		wxYES_NO, this);
	if (answer != wxYES)
	{    
		return;
