...
MountSettingDialog::OnInitDialog(wxInitDialogEvent &e)
{
    ...
    Fairy::LogicModelManager::AnimationSkeletonLinksIterator asmIt = 
        Fairy::LogicModelManager::getSingleton().getAnimationSkeletonLinksIterator(mMountModel->getName());
    ...
    case ID_TEXTCTRL_ATTACHED_MODEL_ONE:
        {
            ...
            if ( NULL == Fairy::LogicModelManager::getSingleton().getTemplateByName(modelName) )
            {
            ...
            if (mAttachedModel1)
            {
                ...
                // 先删除原来的model
                Fairy::LogicModelManager::getSingleton().destroyLogicModel(mAttachedModel1);
            ...
            } while (Fairy::LogicModelManager::getSingleton().getModelByName(newModelName));

            try
            {
                mAttachedModel1 = Fairy::LogicModelManager::getSingleton().createLogicModel(newModelName, modelName);
            }
    ...
    case ID_TEXTCTRL_ATTACHED_MODEL_TWO:
        {
            ...
            if (mAttachedModel2)
            {
                ...
                // 先删除原来的model
                Fairy::LogicModelManager::getSingleton().destroyLogicModel(mAttachedModel2);
            ...
            try
            {
                mAttachedModel2 = Fairy::LogicModelManager::getSingleton().createLogicModel(newModelName, modelName);
            }
            catch(Ogre::Exception& e)
            {
                wxMessageBox(e.getDescription().c_str());
