...
AutoSaveSceneController::AutoSaveSceneController(EffectEditorFrame* frame, float interval) :
mFrame(frame),
mController(NULL),
...
void AutoSaveSceneController::_createController(EffectEditorFrame* frame, float interval)
{
    if (mController)
    {
        Ogre::ControllerManager::getSingleton().destroyController(mController);
