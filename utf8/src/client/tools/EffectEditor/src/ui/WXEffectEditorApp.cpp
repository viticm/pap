...
// ----------------------------------------------------------------------------
    ...
    #pragma implementation "WXEffectEditorApp.h"
#endif
...
// not wxApp)
IMPLEMENT_APP(EffectEditorApp)

// the event tables connect the wxWidgets events with the functions (event
// handlers) which process them.
BEGIN_EVENT_TABLE(EffectEditorApp, wxApp)
    EVT_IDLE(EffectEditorApp::OnIdle)
END_EVENT_TABLE()
...
void
EffectEditorApp::OnIdle(wxIdleEvent &e)
{
    wxApp::OnIdle(e);	
    ...
    {
		if(mMainFrame->GetBulletEditor())
			mMainFrame->GetBulletEditor()->tick();
...
bool EffectEditorApp::_InitImpl()
{
	...
	_locale.Init();
	_locale.AddCatalog(wxT("EffectEditor"));

	// Setup application description
	SetVendorName(wxT("HopeCool"));
	SetClassName(wxT("EffectEditor"));
	SetAppName(wxT("效果编辑器"));
    ...
    mMainFrame = new EffectEditorFrame(NULL, wxID_ANY, GetAppName(),
...
int EffectEditorApp::OnExit()
{
    return wxApp::OnExit();
}

void 
EffectEditorApp::SetKeyboardShortcuts(void)
{
	mMainFrame->SetKeyboardShortcuts();
}

void 
EffectEditorApp::_ExceptionHandler(PEXCEPTION_POINTERS pException)
{
...
bool  
EffectEditorApp::OnExceptionInMainLoop()
{
...
void EffectEditorApp::OnUnhandledException()
{
    wxMessageBox("Unhandled Exception", "OnUnhandledException", wxICON_ERROR);
}

void 
EffectEditorApp::OnFatalException()
{
...
void 
EffectEditorApp::_displayExceptionMessageBox(const Ogre::Exception& e)
{
