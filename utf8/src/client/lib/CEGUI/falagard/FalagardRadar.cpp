...
/*
		// pk对象
		for( int i = 0; i < m_aPointerTimer[ IMAGE_TYPE_PK ]; i ++ )	// 应该同一时间只有一个
		{
...
*/
	...
	int FalagardRadar::GetTypeByName( const String& val )
	{
		...
		{
			nType = FalagardRadar::IMAGE_TYPE_MisCompleteNpc;   // 任务NPC( 完成 )
		}
		else if( val == ( utf8*)"MissNoCompleteNpcImage" )
		{
			nType = FalagardRadar::IMAGE_TYPE_MisNoCompleteNpc; // 任务NPC( 未完成 )						 
		}
		else if( val == ( utf8*)"MissionNpcImage" )
		{
			nType = FalagardRadar::IMAGE_TYPE_AcceptNpc;	    // 任务NPC( 可接 )
		}	
