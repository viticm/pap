...
bool get_kernel_version(kernel_version_t& kernel_version) {
  __ENTER_FUNCTION
    ...
    *dot1++ = '\0';
    if (!util::CStringUtil::string2int16(version, kernel_version.major)) return false;

    char* dot2 = strchr(dot1, '.');
    if (NULL == dot2) return false;
    *dot2++ = '\0';
    if (!util::CStringUtil::string2int16(dot1, kernel_version.minor)) return false;
...
bool do_get_net_info_array(const char* interface_name, const std::vector<net_info_t>& net_info_array) {
  __ENTER_FUNCTION
...
bool get_net_info(const char* interface_name, const net_info_t& net_info) {
  __ENTER_FUNCTION
